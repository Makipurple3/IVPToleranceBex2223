...
```{r FemaleMaleID Check, echo=FALSE}
...
```{r Dyad & Date: Trial, Experiment, DaysSinceStart Dyad, echo=FALSE}

# Step 1: Ensure dates are treated as Date objects and Sort Data by Date
BexClean <- BexClean %>%
  mutate(Date = as.Date(Date, format="%Y-%m-%d")) %>%
  arrange(Date)  # Ensure data is strictly sorted by Date

# Step 2: Calculate DaysSinceStart - Difference in days from the earliest date (including gaps)
BexClean <- BexClean %>%
  mutate(DaysSinceStart = as.numeric(difftime(Date, min(Date, na.rm = TRUE), units = "days")) + 1)

# Step 3: Calculate ExperimentDay - Assign a unique day number to each unique date across the dataset
BexClean <- BexClean %>%
  mutate(ExperimentDay = cumsum(Date != lag(Date, default = first(Date))) + 1)

# Step 4: Verification of ExperimentDay calculation
# Recalculate ExperimentDay to verify correctness
BexClean <- BexClean %>%
  mutate(ExperimentDay_Verified = cumsum(Date != lag(Date, default = first(Date))) + 1)


# Step 5: Create the Trial variable - Each trial has a unique identifier within each dyad, starting from 1
# Group by Dyad to correctly calculate Trial
BexClean <- BexClean %>%
  group_by(Dyad) %>%
...
# Step 6: Create the DyadDay variable - Counts the number of unique experiment days for each dyad separately
# We use dense_rank here within each Dyad to account for the unique experiment days for each dyad
BexClean <- BexClean %>%
  group_by(Dyad) %>%
  arrange(Date, .by_group = TRUE) %>%
  mutate(DyadDay = dense_rank(Date)) %>%
  ungroup()


#Step 7: Create TrialDay variables - Count the number of unique trials for each day for each dyad separetely
BexClean <- BexClean %>%
  group_by(Dyad, Date) %>%
  arrange(Date, Time, .by_group = TRUE) %>%
  mutate(TrialDay = row_number()) %>%
  ungroup()

# Display relevant columns to check
BexClean %>%
  select(Date, Dyad, Trial, DyadDay, TrialDay) %>%
  print()
...
# Summary of Trials and Days per Dyad
...
summary_Day <- BexClean %>%
  group_by(Dyad) %>%
  summarise(Num_Days = max(DyadDay)) %>%
  arrange(desc(Num_Days))

# Combined Summary
combined_summary <- summary_trial %>%
  left_join(summary_Day, by = "Dyad") %>%
  arrange(desc(Num_Trials), desc(Num_Days))

# Print Summary
cat("Total Number of Unique Experiment Days:", n_distinct(BexClean$Date), "\n")
cat("\nCombined Summary:\n")
kable(combined_summary, col.names = c("Dyad", "Amount of Trials", "Number of Days"), caption = "Summary of Trials and Days")
...
```{r DyadResponse TwoResponse & ThreeResponse Frequency, echo=FALSE}
...
# Identify rows with exactly two responses
rows_with_two_entries <- BexClean %>%
  filter(sapply(strsplit(DyadResponse_sorted, ";"), length) == 2)

# Display number of rows with exactly two entries
cat("Number of rows with exactly 2 entries in DyadResponse: ", nrow(rows_with_two_entries), "\n")
...
# Identify rows with more than two responses
rows_with_more_than_two_entries <- BexClean %>%
  filter(sapply(strsplit(DyadResponse_sorted, ";"), length) > 2)

# Display number of rows with more than two entries
cat("Number of rows with more than 2 entries in DyadResponse: ", nrow(rows_with_more_than_two_entries), "\n")
...
```{r DyadResponse unique and multiple combinations and frequencies and plots, echo=FALSE}
...
# Display rows with single responses
print(knitr::kable(single_responses, col.names = c("DyadResponse", "Frequency"), caption = "Rows with Single Responses in DyadResponse", row.names = FALSE))

# Plot the frequency of single dyad responses
ggplot(single_responses, aes(x = reorder(DyadResponse_sorted, -n), y = n)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = n), vjust = -0.3) +
  theme_minimal() +
  labs(
    title = "Frequency of single response in DyadResponse before modification",
    x = "Behaviour produced during a trial",
    y = "Frequency (Amount of trials)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Display rows with multiple responses
print(knitr::kable(multiple_responses, col.names = c("Combination", "Frequency"), caption = "Rows with Multiple Responses in DyadResponse", row.names = FALSE))


# Plot the frequency of multiple dyad response combinations
ggplot(multiple_responses, aes(x = reorder(DyadResponse_sorted, -n), y = n)) +
  geom_bar(stat = "identity", fill = "orange") +
  geom_text(aes(label = n), hjust = -0.3) +
  theme_minimal() +
  labs(
    title = "Frequency of multiple DyadResponse combinations before modification",
    x = "Behaviour produced during a trial",
    y = "Frequency (Amount of trials)"
  ) +
...
```{r plot and Frequency for DyadResponse, echo=FALSE}

# Count unique combinations for more than two entries
if (nrow(rows_with_more_than_two_entries) > 0) {
  all_combinations_more_than_2 <- rows_with_more_than_two_entries %>%
    mutate(Combination = sapply(strsplit(DyadResponse_sorted, ";"), function(x) paste(sort(x), collapse = " & "))) %>%
    count(Combination) %>%
    arrange(desc(n))
  
  # Display unique combinations and counts
  cat("Unique Combinations and Counts for More than 2 Entries:\n")
  for (entry in seq_along(all_combinations_more_than_2$Combination)) {
    cat(sprintf("%-25s %d\n", all_combinations_more_than_2$Combination[entry], all_combinations_more_than_2$n[entry]))
  }
} else {
  cat("No rows with more than 2 entries in DyadResponse.\n")
}
...
(BexClean[beforesummaryrowswithmultipleentries, ])

# Print the number of rows with multiple entries and the total number of rowslength(beforesummaryrowswithmultipleentries), "\n")
...
```{r Dyad Response Unique, echo=FALSE}


# Ensure there are no semicolons in single response entries
BexClean$DyadResponse <- gsub(";", "", BexClean$DyadResponse)

# Summary of unique responses
unique_responses_summary <- table(BexClean$DyadResponse)

# Convert to data frame and sort by Response column
df_unique_responses <- as.data.frame(unique_responses_summary, stringsAsFactors = FALSE)
colnames(df_unique_responses) <- c("Response", "Frequency")
df_unique_responses <- df_unique_responses[order(df_unique_responses$Response), ]

# Print sorted unique responses and counts
cat("Unique Responses and Counts (Sorted Alphabetically):\n")
for (entry in seq_along(df_unique_responses$Response)) {
  cat(sprintf("%-25s %d\n", df_unique_responses$Response[entry], df_unique_responses$Frequency[entry]))
}

# Plot the frequency of unique responses
ggplot(df_unique_responses, aes(x = reorder(Response, -Frequency), y = Frequency)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  geom_text(aes(label = Frequency), vjust = -0.3) +
  theme_minimal() +
  labs(
    title = "Frequency of Unique Responses in DyadResponse",
    x = "Behaviour produced during a trial",
    y = "Frequency (Amount of trials)"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r DyadResponse Looks at partner, echo=FALSE}
# Count occurrences of "Looks at partner" in DyadResponse
looks_at_partner_count <- sum(grepl("Looks at partner", BexClean$DyadResponse))
cat("Number of occurrences of 'Looks at partner' in DyadResponse: ", looks_at_partner_count, "\n")

# Create a backup of the original DyadResponse column
BexClean$DyadResponse_original <- BexClean$DyadResponse

# Delete all occurrences of "Looks at partner" in DyadResponse
BexClean$DyadResponse <- gsub("Looks at partner", "", BexClean$DyadResponse)

# Count the number of rows that have been changed
changed_rows_count <- sum(BexClean$DyadResponse != BexClean$DyadResponse_original)
cat("Number of rows that have been changed: ", changed_rows_count, "\n")

# Drop the backup column if no longer needed
BexClean$DyadResponse_original <- NULL
...
```{r DyadResponse Others, echo=FALSE}

# Ensure data consistency
BexClean <- BexClean %>% mutate(across(everything(), as.character))

# Identify and count occurrences of "Other" in DyadResponse
other_occurrences <- grep("Other", BexClean$DyadResponse)

# Print the number of "Other" occurrences
cat("Number of 'Other' occurrences in DyadResponse: ", length(other_occurrences), "\n")

# Create a data frame with the relevant information
other_occurrences_df <- data.frame(
  Line = other_occurrences,
  MaleID = BexClean$MaleID[other_occurrences],
  FemaleID = BexClean$FemaleID[other_occurrences],
  OtherResponse = BexClean$OtherResponse[other_occurrences]
)

# Display the data frame using kable
kable(other_occurrences_df, col.names = c("Line", "MaleID", "FemaleID", "OtherResponse"),
      caption = "Rows with 'Other' in DyadResponse")
...
```{r DyadResponse Others Treatment Other&NoResponse -, echo=FALSE}

# Function to find and count specific combinations in the dataframe without removing any rows
find_and_remove_other_no_response <- function(df) {
  # Filter rows where DyadResponse is 'Other' and OtherResponse is 'No Response'
  filtered_rows <- df %>%
    filter(tolower(DyadResponse) == "other" & tolower(OtherResponse) == "no response")
  
  # Get the row numbers of these filtered rows
  row_numbers <- which(tolower(df$DyadResponse) == "other" & tolower(df$OtherResponse) == "no response")
  
  # Count the number of such combinations
  count_combinations <- length(row_numbers)
  ...
  rows_changed <- nrow(df) - nrow(df_updated)
  
  # Count the remaining occurrences of 'Other' in DyadResponse
  remaining_other_count <- sum(grepl("Other", df_updated$DyadResponse))
  
  # Return updated dataframe, row numbers, count of combinations, number of rows changed, and remaining 'Other' occurrences
  return(list(df_updated, row_numbers, count_combinations, rows_changed, remaining_other_count))
}

# Assuming BexClean is your dataframe
result <- find_and_remove_other_no_response(BexClean)

# Extract the updated dataframe, row numbers, count of combinations, rows changed, and remaining 'Other' occurrences
BexClean <- result[[1]]
row_numbers <- result[[2]]
count_combinations <- result[[3]]
rows_changed <- result[[4]]
remaining_other_count <- result[[5]]

# Display the results
cat("Number of combinations with 'Other' in DyadResponse and 'No Response' in OtherResponse: ", count_combinations, "\n")
cat("Row numbers: ", paste(row_numbers, collapse = ", "), "\n")
cat("Number of rows that have been changed: ", rows_changed, "\n")
cat("Number of occurrences of 'Other' in DyadResponse left: ", remaining_other_count, "\n")

# Remove any rows where DyadResponse contains 'Other'
BexClean <- BexClean %>% filter(!grepl("Other", DyadResponse, ignore.case = TRUE))

```

* Then I will treat the remaining 13 lines according to the rules I explained

```{r DyadResponse Others Treatment - Rows to Change, echo=FALSE}
# Function to update specific rows in the dataframe based on given criteria
update_rows <- function(df) {
  # Define criteria for specific rows
  criteria_286 <- (df$MaleID == "Kom" & df$FemaleID == "Oort" & df$OtherResponse == "Ooet scream while at the box and Kom get the corm of both")
  criteria_300 <- (df$MaleID == "Kom" & df$FemaleID == "Oort" & df$OtherResponse == "Both at boxes; Kom touching the box of Oort")
  criteria_304 <- (df$MaleID == "Kom" & df$FemaleID == "Oort" & df$OtherResponse == "Kom touching Oort's box. Oort came after 30 sec to her own box.")
  criteria_1484 <- (df$MaleID == "Sey" & df$FemaleID == "Sirk" & df$OtherResponse == "wait for her. moved back and waited for sirk to approach and came back")
  criteria_1741 <- (df$MaleID == "Sho" & df$FemaleID == "Ginq" & df$OtherResponse == "Sho took one of her corn")
  criteria_2102 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "Xia ate corn from both boxes")
  criteria_2103 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "Xia stolen some pieces from piep box")
  criteria_2104 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "Xia immediately go for piep box and eat her corn then eat his.")
  criteria_2105 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "piep got 2 pieces out of 3. Xia took one corn")
  criteria_2170 <- (df$MaleID == "Piep" & df$FemaleID == "Xia" & df$OtherResponse == "fem ag male")
  criteria_2184 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "Neither ID approach")
  criteria_2101 <- (df$MaleID == "Xia" & df$FemaleID == "Piep" & df$OtherResponse == "aggression")
  criteria_2729 <- (df$MaleID == "Xin" & df$FemaleID == "Ouli" & df$OtherResponse == "opened for oerw")
  
  # Update the specific rows without removing them
  df <- df %>%
    mutate(
      DyadResponse = case_when(
        criteria_286 ~ "Tolerance",
        criteria_300 ~ "Tolerance",
        criteria_304 ~ "Tolerance",
        criteria_1484 ~ "Tolerance",
        criteria_1741 ~ "Tolerance",
        criteria_2102 ~ "Tolerance",
        criteria_2103 ~ "Tolerance",
        criteria_2104 ~ "Tolerance",
        criteria_2105 ~ "Tolerance",
        criteria_2170 ~ "Female aggress male",
        criteria_2184 ~ "Not approaching",
        TRUE ~ DyadResponse
      ),
      SpecialBehaviour = case_when(
        criteria_286 ~ "Kom;Ap;Sf;Oort,Oort;Rt;Sc",
        criteria_304 ~ "Oort;Ap;After30",
        criteria_1484 ~ "Sirk;ApproachWithPartner",
        criteria_1741 ~ "Sho;Sf;Ginq",
        (criteria_2102 | criteria_2103 | criteria_2104 | criteria_2105) ~ "Xia;Sf;Piep",
        TRUE ~ SpecialBehaviour
      ),
      GotCorn = case_when(
        criteria_286 ~ "No;Oort",
        criteria_1741 ~ "No;Ginq",
        (criteria_2102 | criteria_2103 | criteria_2104 | criteria_2105) ~ "No;Piep",
        TRUE ~ GotCorn
      )
    )
  
  # Count the number of rows changed
  rows_changed <- sum(criteria_286 | criteria_300 | criteria_304 | criteria_1484 | criteria_1741 |
                      criteria_2102 | criteria_2103 | criteria_2104 | criteria_2105 | criteria_2170 | criteria_2184 |
                      criteria_2101 | criteria_2729)
  
  # Return the updated dataframe and number of rows changed
  return(list(df, rows_changed))
}

# Process the dataframe to update rows based on criteria
result <- update_rows(BexClean)

# Extract the updated dataframe and rows changed
BexClean <- result[[1]]
rows_changed <- result[[2]]

# Display the number of rows changed
cat("Number of rows changed: ", rows_changed, "\n")
...
```{r DyadResponse Hierarchy Tolerance & Aggression, echo=FALSE}

# Function to handle Male aggress female or Female aggress male and Tolerance cases
handle_aggression_tolerance <- function(df) {
  # Create SimultaneousTouch variable
  df <- df %>%
    mutate(SimultaneousTouch = ifelse(grepl("Male aggress female", DyadResponse) & grepl("Tolerance", DyadResponse) |
                                      grepl("Female aggress male", DyadResponse) & grepl("Tolerance", DyadResponse), "Yes", "No"))
  
  # Count the number of cases with Tolerance & Male aggress female
  count_tolerance_male_aggression <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  
  # Count the number of cases with Tolerance & Female aggress male
  count_tolerance_female_aggression <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  
  # Remove 'Tolerance' when 'Male aggress female' or 'Female aggress male' is present
  df <- df %>%
    mutate(DyadResponse = case_when(
      grepl("Male aggress female", DyadResponse) & grepl("Tolerance", DyadResponse) ~ gsub("Tolerance", "", DyadResponse),
      grepl("Female aggress male", DyadResponse) & grepl("Tolerance", DyadResponse) ~ gsub("Tolerance", "", DyadResponse),
      TRUE ~ DyadResponse
    ))
  
  # Count the number of rows changed
  rows_changed <- count_tolerance_male_aggression + count_tolerance_female_aggression
  
  # Count remaining lines with both Aggression and Tolerance
  remaining_aggression_tolerance <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Tolerance", df$DyadResponse)) +
                                    sum(grepl("Female aggress male", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, count_tolerance_male_aggression, count_tolerance_female_aggression, rows_changed, remaining_aggression_tolerance))
}

# Apply the function to the dataframe
result <- handle_aggression_tolerance(BexClean)

# Extract the updated dataframe and counts
BexClean_updated <- result[[1]]
count_tolerance_male_aggression <- result[[2]]
count_tolerance_female_aggression <- result[[3]]
rows_changed <- result[[4]]
remaining_aggression_tolerance <- result[[5]]

# Display the results
cat("Number of cases with Tolerance & Male aggress female: ", count_tolerance_male_aggression, "\n")
cat("Number of cases with Tolerance & Female aggress male: ", count_tolerance_female_aggression, "\n")
cat("Number of rows changed: ", rows_changed, "\n")
cat("Number of lines remaining with both Aggression and Tolerance: ", remaining_aggression_tolerance, "\n")
...
```{r DyadResponse Hierarchy Tolerance & Loosing interest, echo=FALSE}

# Function to handle Tolerance & Losing interest cases
handle_tolerance_losing_interest <- function(df) {
  # Count the number of cases with Tolerance & Losing interest
  count_tolerance_losing_interest <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Losing interest", df$DyadResponse))
  
  # Remove 'Losing interest' when 'Tolerance' is present
  df <- df %>%
    mutate(DyadResponse = case_when(
      grepl("Tolerance", DyadResponse) & grepl("Losing interest", DyadResponse) ~ gsub("Losing interest", "", DyadResponse),
      TRUE ~ DyadResponse
    ))
  
  # Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)
  
  # Count the number of rows changed
  rows_changed <- count_tolerance_losing_interest
  
  # Count remaining lines with both Tolerance and Losing interest
  remaining_tolerance_losing_interest <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Losing interest", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, count_tolerance_losing_interest, rows_changed, remaining_tolerance_losing_interest))
}

# Apply the function to the dataframe
result_tolerance_losing_interest <- handle_tolerance_losing_interest(BexClean)

# Extract the updated dataframe and counts
BexClean_updated_losing_interest <- result_tolerance_losing_interest[[1]]
count_tolerance_losing_interest <- result_tolerance_losing_interest[[2]]
rows_changed_losing_interest <- result_tolerance_losing_interest[[3]]
remaining_tolerance_losing_interest <- result_tolerance_losing_interest[[4]]

# Display the results
cat("Number of cases with Tolerance & Losing interest: ", count_tolerance_losing_interest, "\n")
cat("Number of rows changed: ", rows_changed_losing_interest, "\n")
cat("Number of lines remaining with both Tolerance and Losing interest: ", remaining_tolerance_losing_interest, "\n")
...
```{r DyadResponse Hierarchy Tolerance & Intrusion, echo=FALSE}

# Function to handle Tolerance & Intrusion cases
handle_tolerance_intrusion <- function(df) {
  # Count the number of cases with Tolerance & Intrusion
  count_tolerance_intrusion <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Intrusion", df$DyadResponse))
  
  # Create a variable called Intrusion
  df <- df %>%
    mutate(Intrusion = ifelse(grepl("Intrusion", DyadResponse), "Yes", "No"))
  
  # Remove 'Intrusion' when 'Tolerance' is present
  df <- df %>%
    mutate(DyadResponse = case_when(
      grepl("Tolerance", DyadResponse) & grepl("Intrusion", DyadResponse) ~ gsub("Intrusion", "", DyadResponse),
      TRUE ~ DyadResponse
    ))
  ...
  # Count the number of rows changed
  rows_changed <- count_tolerance_intrusion
  
  # Count remaining lines with both Tolerance and Intrusion
  remaining_tolerance_intrusion <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Intrusion", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, count_tolerance_intrusion, rows_changed, remaining_tolerance_intrusion))
}

# Apply the function to the dataframe
result_tolerance_intrusion <- handle_tolerance_intrusion(BexClean_updated_losing_interest)

# Extract the updated dataframe and counts
BexClean_updated_intrusion <- result_tolerance_intrusion[[1]]
count_tolerance_intrusion <- result_tolerance_intrusion[[2]]
rows_changed_intrusion <- result_tolerance_intrusion[[3]]
remaining_tolerance_intrusion <- result_tolerance_intrusion[[4]]

# Display the results
cat("Number of cases with Tolerance & Intrusion: ", count_tolerance_intrusion, "\n")
cat("Number of rows changed: ", rows_changed_intrusion, "\n")
cat("Number of lines remaining with both Tolerance and Intrusion: ", remaining_tolerance_intrusion, "\n")
...
```{r DyadResponse Hierarchy Not approaching & Loosing interest, echo=FALSE}
# Function to replace "Losing interest" with "Not approaching" and clean up the DyadResponse column
replace_losing_interest <- function(df) {
  # Count initial duplicates
  initial_duplicates <- sum(grepl("\\bNot approaching\\b", df$DyadResponse))
  cat("Initial duplicates of 'Not Approaching': ", initial_duplicates, "\n")
  
  # Step 1: Replace "Losing interest" with "Not approaching"
  df <- df %>%
    mutate(DyadResponse = gsub("\\bLosing interest\\b", "Not approaching", DyadResponse))
  
  # Step 2: Remove any duplicate "Not approaching" entries
  df <- df %>%
    mutate(DyadResponse = gsub("(\\bNot approaching\\b\\s*;\\s*)+Not approaching\\b", "Not approaching", DyadResponse),
           DyadResponse = gsub("\\bNot approaching;\\s*$", "Not approaching", DyadResponse)) # Remove trailing semicolon if present

  # Step 3: Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)
  
  # Count final duplicates
  final_duplicates <- sum(grepl("\\bNot approaching\\b", df$DyadResponse))
  cat("Final duplicates of 'Not Approaching': ", final_duplicates, "\n")

  # Display the counts
  final_not_approaching_count <- sum(grepl("\\bNot approaching\\b", df$DyadResponse))
  cat("Final number of 'Not approaching' occurrences after replacing 'Losing interest': ", final_not_approaching_count, "\n")
  cat("Number of rows with 'Losing interest' remaining: 0\n")
  
  return(df)
}

# Apply the function to the dataframe
BexClean <- replace_losing_interest(BexClean)
...
```{r DyadResponse hierarchy Loosing interest & Not approaching, echo =FALSE}
...
```{r DyadResponse Hierarchy Intrusion & not Approaching , echo=FALSE}

# F# Function to handle Intrusion and Not approaching cases
handle_intrusion_not_approaching <- function(df) {
  # Count initial duplicates
  initial_duplicates <- sum(grepl("Not approaching\\s*;\\s*Not approaching", df$DyadResponse))
  cat("Initial duplicates of 'Not Approaching; Not Approaching': ", initial_duplicates, "\n")

  # Count the number of cases with Intrusion & Not Approaching
  count_intrusion_not_approaching <- sum(grepl("Intrusion", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  
  # Remove 'Not Approaching' when both 'Intrusion' and 'Not Approaching' are present
  df <- df %>%
    mutate(DyadResponse = case_when(
      grepl("Intrusion", df$DyadResponse) & grepl("Not approaching", df$DyadResponse) ~ gsub("Not approaching", "", DyadResponse),
      TRUE ~ DyadResponse
    ))

  # Remove duplicate "Not approaching" entries
  df <- df %>%
    mutate(DyadResponse = gsub("(Not approaching\\s*;\\s*Not approaching)+", "Not approaching", DyadResponse),
           DyadResponse = gsub("Not approaching;\\s*$", "Not approaching", DyadResponse)) # Remove trailing semicolon if present

  # Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)
  
  # Count final duplicates
  final_duplicates <- sum(grepl("Not approaching\\s*;\\s*Not approaching", df$DyadResponse))
  cat("Final duplicates of 'Not Approaching; Not Approaching': ", final_duplicates, "\n")

  # Count the remaining lines with both Intrusion and Not Approaching
  remaining_intrusion_not_approaching <- sum(grepl("Intrusion", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))

  # Display the counts
  cat("Number of cases with Intrusion & Not Approaching: ", count_intrusion_not_approaching, "\n")
  cat("Number of lines remaining with both Intrusion and Not Approaching: ", remaining_intrusion_not_approaching, "\n")
  
  return(df)
}

# Apply the function to the dataframe
BexClean <- handle_intrusion_not_approaching(BexClean)

```

* **Not approaching & Distracted**: I will keep **Not approaching**: **Not approaching > Distracted**

```{r DyadResponse Hierarchy Not approaching & Distracted , echo=FALSE}
## Function to count and replace occurrences of "Distracted" with "Not approaching"
replace_distracted_with_not_approaching <- function(df) {
  # Count initial duplicates
  initial_duplicates <- sum(grepl("Not approaching\\s*;\\s*Not approaching", df$DyadResponse))
  cat("Initial duplicates of 'Not Approaching; Not Approaching': ", initial_duplicates, "\n")

  # Count initial occurrences
  initial_distracted_count <- sum(grepl("Distracted", df$DyadResponse))
  initial_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))

  # Replace "Distracted" with "Not approaching"
  df <- df %>%
    mutate(DyadResponse = gsub("Distracted", "Not approaching", DyadResponse))

  # Ensure no duplicate "Not approaching" entries
  df <- df %>%
    mutate(DyadResponse = gsub("(Not approaching\\s*;\\s*Not approaching)+", "Not approaching", DyadResponse))
  ...
  final_distracted_count <- sum(grepl("Distracted", df$DyadResponse))
  final_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))

  # Calculate the number of changes made
  distracted_changes <- initial_distracted_count - final_distracted_count
  not_approaching_changes <- final_not_approaching_count - initial_not_approaching_count

  # Display the counts of changes made
  cat("Number of changes made to replace 'Distracted' with 'Not approaching': ", distracted_changes, "\n")
  cat("Number of changes in 'Not approaching' entries: ", not_approaching_changes, "\n")

  return(df)
}

# Apply the function to the dataframe
BexClean <- replace_distracted_with_not_approaching(BexClean)
...
```{r DyadResponse Hierarchy Male Aggress Female & Female Aggress male with Not Approaching, echo=FALSE}

# Function to count the number of cases with Aggression & Not Approaching
count_aggression_not_approaching <- function(df) {
  count_male_aggress_female_not_approaching <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  count_female_aggress_male_not_approaching <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  total_count <- count_male_aggress_female_not_approaching + count_female_aggress_male_not_approaching
  return(list(count_male_aggress_female_not_approaching, count_female_aggress_male_not_approaching, total_count))
}

# Apply the function to the dataframe
counts_aggression_not_approaching <- count_aggression_not_approaching(BexClean)

# Extract counts
count_male_aggress_female_not_approaching <- counts_aggression_not_approaching[[1]]
count_female_aggress_male_not_approaching <- counts_aggression_not_approaching[[2]]
total_aggression_not_approaching <- counts_aggression_not_approaching[[3]]

# Display the result
cat("Number of cases with Male Aggress Female & Not Approaching: ", count_male_aggress_female_not_approaching, "\n")
cat("Number of cases with Female Aggress Male & Not Approaching: ", count_female_aggress_male_not_approaching, "\n")
cat("Total number of cases with Aggression & Not Approaching: ", total_aggression_not_approaching, "\n")

# Function to remove 'Not Approaching' when both 'Aggression' (Male/Female aggress) and 'Not Approaching' are present in DyadResponse
remove_not_approaching_with_aggression <- function(df) {
  # Remove 'Not Approaching' when Male aggress female is present
  changes_made_male <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Male aggress female", DyadResponse) & grepl("Not approaching", DyadResponse), 
                                 gsub("Not approaching", "", DyadResponse), 
                                 DyadResponse))
  
  # Remove 'Not Approaching' when Female aggress male is present
  changes_made_female <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Female aggress male", DyadResponse) & grepl("Not approaching", DyadResponse), 
                                 gsub("Not approaching", "", DyadResponse), 
                                 DyadResponse))
  ...
  # Count remaining lines with both Aggression (Male/Female aggress) and Not Approaching
  remaining_male_aggress_female_not_approaching <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  remaining_female_aggress_male_not_approaching <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, changes_made_male, changes_made_female, remaining_male_aggress_female_not_approaching, remaining_female_aggress_male_not_approaching))
}

# Apply the function to the dataframe
result_remove_not_approaching <- remove_not_approaching_with_aggression(BexClean)

# Extract the updated dataframe and counts
BexClean_updated <- result_remove_not_approaching[[1]]
changes_made_male <- result_remove_not_approaching[[2]]
changes_made_female <- result_remove_not_approaching[[3]]
remaining_male_aggress_female_not_approaching <- result_remove_not_approaching[[4]]
remaining_female_aggress_male_not_approaching <- result_remove_not_approaching[[5]]

# Display the count of changes made and remaining occurrences
cat("Number of changes made to remove 'Not Approaching' with 'Male aggress female': ", changes_made_male, "\n")
cat("Number of changes made to remove 'Not Approaching' with 'Female aggress male': ", changes_made_female, "\n")
cat("Number of remaining cases with Male Aggress Female & Not Approaching: ", remaining_male_aggress_female_not_approaching, "\n")
cat("Number of remaining cases with Female Aggress Male & Not Approaching: ", remaining_female_aggress_male_not_approaching, "\n")
...
```{r DyadResponse Hierarchy Tolerance & FeMale aggres FeMale, echo=FALSE}

# Ensure DyadResponse is a character column and exists in the dataframe
if (!"DyadResponse" %in% colnames(BexClean)) {
  stop("DyadResponse column is not found in the dataframe.")
}
BexClean$DyadResponse <- as.character(BexClean$DyadResponse)

# Function to count the number of cases with Aggression & Tolerance
count_aggression_tolerance <- function(df) {
  count_male_aggress_female_tolerance <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  count_female_aggress_male_tolerance <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  total_count <- count_male_aggress_female_tolerance + count_female_aggress_male_tolerance
  return(list(count_male_aggress_female_tolerance, count_female_aggress_male_tolerance, total_count))
}

# Apply the function to the dataframe
counts_aggression_tolerance <- count_aggression_tolerance(BexClean)

# Extract counts
count_male_aggress_female_tolerance <- counts_aggression_tolerance[[1]]
count_female_aggress_male_tolerance <- counts_aggression_tolerance[[2]]
total_aggression_tolerance <- counts_aggression_tolerance[[3]]

# Display the result
cat("Number of cases with Male Aggress Female & Tolerance: ", count_male_aggress_female_tolerance, "\n")
cat("Number of cases with Female Aggress Male & Tolerance: ", count_female_aggress_male_tolerance, "\n")

# Function to remove 'Aggression' when both 'Aggression' (Male/Female aggress) and 'Tolerance' are present in DyadResponse
remove_aggression_with_tolerance <- function(df) {
  # Check if DyadResponse column exists within the dataframe
  if (!"DyadResponse" %in% colnames(df)) {
    stop("DyadResponse column is not found in the dataframe.")
  }
  df$DyadResponse <- as.character(df$DyadResponse)
  
  # Remove 'Male aggress female' when Tolerance is present
  changes_made_male <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Male aggress female", DyadResponse) & grepl("Tolerance", DyadResponse), 
                                 gsub("Male aggress female", "", DyadResponse), 
                                 DyadResponse))
  
  # Remove 'Female aggress male' when Tolerance is present
  changes_made_female <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Female aggress male", DyadResponse) & grepl("Tolerance", DyadResponse), 
                                 gsub("Female aggress male", "", DyadResponse), 
                                 DyadResponse))
  
  # Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)
  
  # Count remaining lines with both Aggression (Male/Female aggress) and Tolerance
  remaining_male_aggress_female_tolerance <- sum(grepl("Male aggress female", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  remaining_female_aggress_male_tolerance <- sum(grepl("Female aggress male", df$DyadResponse) & grepl("Tolerance", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, changes_made_male, changes_made_female, remaining_male_aggress_female_tolerance, remaining_female_aggress_male_tolerance))
}

# Apply the function to the dataframe
result_remove_aggression <- remove_aggression_with_tolerance(BexClean)

# Extract the updated dataframe and counts
BexClean <- result_remove_aggression[[1]]
changes_made_male <- result_remove_aggression[[2]]
changes_made_female <- result_remove_aggression[[3]]
remaining_male_aggress_female_tolerance <- result_remove_aggression[[4]]
remaining_female_aggress_male_tolerance <- result_remove_aggression[[5]]

# Display the count of changes made and remaining occurrences
cat("Number of changes made to remove 'Male aggress female' with 'Tolerance': ", changes_made_male, "\n")
cat("Number of changes made to remove 'Female aggress male' with 'Tolerance': ", changes_made_female, "\n")
cat("Number of remaining cases with Male Aggress Female & Tolerance: ", remaining_male_aggress_female_tolerance, "\n")
cat("Number of remaining cases with Female Aggress Male & Tolerance: ", remaining_female_aggress_male_tolerance, "\n")
...
```{r DyadResponse Hierarchy Aggression, echo=FALSE}


# Function to create Aggression variable based on occurrences of Male and Female aggression
create_aggression_variable <- function(df) {
  # Create Aggression variable
  df <- df %>%
    mutate(Aggression = ifelse(grepl("Male aggress female", DyadResponse) | grepl("Female aggress male", DyadResponse), "Yes", "No"))
  
  # Count occurrences of Male aggress female and Female aggress male
  count_male_aggress_female <- sum(grepl("Male aggress female", df$DyadResponse))
  count_female_aggress_male <- sum(grepl("Female aggress male", df$DyadResponse))
  total_aggression_count <- sum(df$Aggression == "Yes")
  
  # Return updated dataframe and counts
  return(list(df, count_male_aggress_female, count_female_aggress_male, total_aggression_count))
}

# Apply the function to the dataframe
result_aggression <- create_aggression_variable(BexClean)

# Extract the updated dataframe and counts
BexClean_updated_aggression <- result_aggression[[1]]
count_male_aggress_female <- result_aggression[[2]]
count_female_aggress_male <- result_aggression[[3]]
total_aggression_count <- result_aggression[[4]]

# Display the results
cat("Number of cases with Male aggress female: ", count_male_aggress_female, "\n")
cat("Number of cases with Female aggress male: ", count_female_aggress_male, "\n")
cat("Total number of aggression cases: ", total_aggression_count, "\n")
...
```{r DyadResponse Hierarchy Tolerance & Not Approaching  , echo=FALSE}

# Corrected function for handling Tolerance & Not Approaching cases
handle_tolerance_not_approaching <- function(df) {
  # Count the number of cases with Tolerance & Not Approaching
  count_tolerance_not_approaching <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  
  # Create a variable called Hesitant
  df <- df %>%
    mutate(Hesitant = ifelse(grepl("Tolerance", DyadResponse) & grepl("Not approaching", DyadResponse), "Yes", "No"))
  
  # Remove 'Not approaching' when 'Tolerance' is present
  df <- df %>%
    mutate(DyadResponse = case_when(
      grepl("Tolerance", DyadResponse) & grepl("Not approaching", DyadResponse) ~ gsub("Not approaching", "", DyadResponse),
      TRUE ~ DyadResponse
    ))
  ...
  # Count the number of rows changed
  rows_changed <- count_tolerance_not_approaching
  
  # Count remaining lines with both Tolerance and Not Approaching
  remaining_tolerance_not_approaching <- sum(grepl("Tolerance", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, count_tolerance_not_approaching, rows_changed, remaining_tolerance_not_approaching))
}

# Apply the function to the dataframe
result_tolerance_not_approaching <- handle_tolerance_not_approaching(BexClean)

# Extract the updated dataframe and counts
BexClean_updated_tolerance_not_approaching <- result_tolerance_not_approaching[[1]]
count_tolerance_not_approaching <- result_tolerance_not_approaching[[2]]
rows_changed_tolerance_not_approaching <- result_tolerance_not_approaching[[3]]
remaining_tolerance_not_approaching <- result_tolerance_not_approaching[[4]]

# Display the results
cat("Number of cases with Tolerance & Not Approaching: ", count_tolerance_not_approaching, "\n")
cat("Number of rows changed: ", rows_changed_tolerance_not_approaching, "\n")
cat("Number of lines remaining with both Tolerance and Not Approaching: ", remaining_tolerance_not_approaching, "\n")
...
```{r not apporaching not approaching treatment, echo=FALSE}
# Step 1: Count occurrences of "Not approaching Not approaching"
count_not_approaching_duplicates <- function(df) {
  count <- sum(grepl("Not\\s+[Aa]pproaching\\s+Not\\s+[Aa]pproaching", df$DyadResponse))
  cat("Count of 'Not approaching Not approaching': ", count, "\n")
  return(count)
}

# Apply the function to count the duplicates
initial_count <- count_not_approaching_duplicates(BexClean)

# Step 2: Function to remove duplicate "Not approaching Not approaching" occurrences
remove_duplicate_not_approaching <- function(df) {
  # Use regex to remove all variations of "Not approaching Not approaching"
  df <- df %>%
    mutate(DyadResponse = gsub("\\bNot\\s+[Aa]pproaching\\s+Not\\s+[Aa]pproaching\\b", "Not approaching", DyadResponse))
  
  return(df)
}

# Apply the function to remove duplicates
BexClean <- remove_duplicate_not_approaching(BexClean)

# Step 3: Check the remaining duplicates after removal
remaining_count <- count_not_approaching_duplicates(BexClean)

# Final check to confirm all duplicates are removed
final_check <- function(df) {
  remaining_count <- count_not_approaching_duplicates(df)
  if (remaining_count == 0) {
    cat("Remaining duplicates of 'Not approaching Not approaching': 0\n")
  } else {
    cat("There are still", remaining_count, "'Not approaching Not approaching' duplicates remaining.\n")
  }
}

# Run the final check
final_check(BexClean)

```


```{r check and state of dyadresponse, echo=FALSE}

# Ensure DyadResponse is a character column
BexClean$DyadResponse <- as.character(BexClean$DyadResponse)

# Function to extract unique keywords from DyadResponse
extract_unique_keywords <- function(df) {
  keywords <- unlist(strsplit(df$DyadResponse, ";"))
  keywords <- trimws(keywords)
  unique_keywords <- table(keywords)
  unique_keywords_df <- as.data.frame(unique_keywords)
  colnames(unique_keywords_df) <- c("Keyword", "Frequency")  # Corrected parentheses
  unique_keywords_df <- unique_keywords_df[order(-unique_keywords_df$Frequency), ]
  return(unique_keywords_df)
}

# Extract unique keywords
unique_keywords_df <- extract_unique_keywords(BexClean)

# Display the table of unique keywords and their frequencies
knitr::kable(unique_keywords_df, caption = "Table of Unique Keywords and Their Frequencies in DyadResponse")

# Plot for unique keywords
keyword_plot <- ggplot(unique_keywords_df, aes(x = reorder(Keyword, -Frequency), y = Frequency, fill = Keyword)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Frequency), vjust = -0.3) +
  theme_minimal() +
  labs(title = "Frequency of Unique Keywords in DyadResponse", x = "Keyword", y = "Frequency") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_brewer(palette = "Set3")

# Print the keyword plot
print(keyword_plot)

# Create a table of unique DyadResponse entries and their counts
unique_dyad_response_table <- BexClean %>%
  count(DyadResponse) %>%
  arrange(desc(n))

# Filter the table to include only entries with more than 10 occurrences
filtered_dyad_response_table <- unique_dyad_response_table %>%
  filter(n > 10)

# Display the table of unique DyadResponse entries with more than 10 occurrences
knitr::kable(filtered_dyad_response_table, caption = "Table of Unique DyadResponse Entries with More Than 10 Occurrences")

# Plot for unique combinations with more than 10 occurrences
dyad_response_plot <- ggplot(filtered_dyad_response_table, aes(x = reorder(DyadResponse, -n), y = n, fill = DyadResponse)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = n), vjust = -0.3) +
  theme_minimal() +
  labs(title = "Frequency of Unique DyadResponse Entries with More Than 10 Occurrences", x = "DyadResponse", y = "Frequency") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_brewer(palette = "Set3")

# Print the dyad response plot
print(dyad_response_plot)
...
```{r Structure of BexClean, echo=FALSE}

#Check of state of new BexClean
str(BexClean)


# Summary of NA values per column
na_summary <- colSums(is.na(BexClean))
print(na_summary)
...
```{r updates of variables before reordering, echo=FALSE}

# Step 1: Ensure dates are treated as Date objects and sorted correctly
BexClean <- BexClean %>%
  mutate(Date = as.Date(Date, format="%Y-%m-%d")) %>%
  arrange(Date) %>%
  mutate(
...
# Step 2: Create Time, Period, and Hour variables
BexClean <- BexClean %>%
  mutate(
    Time = as.POSIXct(Time, format = "%H:%M:%S"),  # Convert Time back to POSIXct
    Period = case_when(
      between(hour(Time), 6, 8) ~ "6 to 8",
      between(hour(Time), 8, 10) ~ "8 to 10",
      between(hour(Time), 10, 12) ~ "10 to 12",
    ...
    Hour = format(floor_date(Time, "hour"), "%H:%M:%S")  # Create and format Hour variable
  )



# Step 3: Create Male, Female, and Dyad variables
BexClean <- BexClean %>%
  mutate(
    Male = ifelse(nchar(MaleID) == 3, MaleID, ifelse(nchar(FemaleID) == 3, FemaleID, NA)),
    Female = ifelse(nchar(FemaleID) == 4, FemaleID, ifelse(nchar(MaleID) == 4, MaleID, NA)),
...
# Step 4: Ensure DyadDistance is numeric and then create the Proximity variable
BexClean <- BexClean %>%
  mutate(
    DyadDistance = as.numeric(DyadDistance),  # Convert DyadDistance to numeric
    Proximity = case_when(
      DyadDistance == 0 ~ "Contact",
      between(DyadDistance, 1, 2) ~ "1-2",
      between(DyadDistance, 3, 4) ~ "3-4",
      between(DyadDistance, 5, 10) ~ "5+",
      TRUE ~ NA_character_
    )
  )

# Step 5: Recalculate the AmountAudience variable


# Step 2: Splitting audience members by delimiters and trimming whitespace
BexClean$AudienceList <- strsplit(BexClean$Audience, "[;,.]")

# Step 3: Cleaning the list by removing any extra spaces
BexClean$AudienceList <- lapply(BexClean$AudienceList, function(x) trimws(x))

# Step 4: Calculating the number of unique audience members with special handling
BexClean$AmountAudience <- sapply(BexClean$AudienceList, function(x) {
  # Special handling for "No audience" and "UnidentifiedAudience"
  if ("No audience" %in% x) {
    return(0)  # Assign 0 for "No audience"
  } else if ("UnidentifiedAudience" %in% x) {
    return(-1)  # Assign -1 for "UnidentifiedAudience"
  } else {
    return(length(unique(x)))  # Count the number of unique valid audience members
  }
})

# Step 5: Reconstructing the Audience column as a cleaned string
BexClean$Audience <- sapply(BexClean$AudienceList, function(x) paste(unique(x), collapse = "; "))

# Optional: Remove the intermediate AudienceList column
BexClean$AudienceList <- NULL
...
```{r Updating variables for reorganization of BexClean, echo=FALSE}
...
# Step 2: Recalculate DaysSinceStart
BexClean <- BexClean %>%
  mutate(DaysSinceStart = as.numeric(difftime(Date, min(Date, na.rm = TRUE), units = "days")) + 1)

# Step 3: Recalculate ExperimentDay
BexClean <- BexClean %>%
  mutate(ExperimentDay = cumsum(Date != lag(Date, default = first(Date))) + 1)

# Step 4: Recalculate Trial (depends on Date and Time within each Dyad)
BexClean <- BexClean %>%
  group_by(Dyad) %>%
  arrange(Date, Time, .by_group = TRUE) %>%
  mutate(Trial = row_number()) %>%
...
# Step 5: Recalculate DyadDay (depends on Date within each Dyad)
BexClean <- BexClean %>%
  group_by(Dyad) %>%
  arrange(Date, .by_group = TRUE) %>%
  mutate(DyadDay = dense_rank(Date)) %>%
...
# Step 6: Create the TrialDay variable - Counts the number of unique trials for each day for each dyad separately
BexClean <- BexClean %>%
  group_by(Dyad, Date) %>%
  arrange(Date, Time, .by_group = TRUE) %>%
  mutate(TrialDay = row_number()) %>%
...
```{r Reorganizing BexClean, echo=FALSE}
# Reordering the columns of BexClean

BexClean <- BexClean %>%
  select(DaysSinceStart, ExperimentDay, Date, Month, DyadDay, TrialDay, Trial, Time, Hour, Period, Male, Female, Dyad, 
         DyadDistance, Proximity, DyadResponse,IDIndividual1, Intrusion, IntruderID, SpecialBehaviour, 
         Audience, AmountAudience, Context, SpecialBehaviour)
...
BexClean$DyadDistance <- as.numeric(BexClean$DyadDistance)
...
if("DyadDistance" %in% colnames(BexClean)) {
    ...
    # Plot using Date with y-axis limit set to 5
    plot <- ggplot(data = BexClean, aes(x = Date, y = DyadDistance, group = Dyad, color = Dyad)) +
      ...
      ylim(0, 5)  # Set y-axis limit to range from 0 to 5
...
# Summarize to get overall mean and SD per Dyad
summary_stats <- BexClean %>%
  group_by(Dyad) %>%
  summarise(
    Overall_Mean_DyadDistance = mean(DyadDistance, na.rm = TRUE),
    SD_DyadDistance = sd(DyadDistance, na.rm = TRUE)
  ) %>%
  arrange(Overall_Mean_DyadDistance)  # Order by mean distance

# Reorder Dyad factor based on Overall_Mean_DyadDistance
summary_stats$Dyad <- factor(summary_stats$Dyad, levels = summary_stats$Dyad)

# Single Plot with Mean and Error Bars for Each Dyad
summary_plot <- ggplot(data = summary_stats, aes(x = Dyad, y = Overall_Mean_DyadDistance, fill = Dyad)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = pmax(0, Overall_Mean_DyadDistance - SD_DyadDistance), 
                    ymax = Overall_Mean_DyadDistance + SD_DyadDistance), width = 0.2) +
  geom_text(aes(label = round(SD_DyadDistance, 2), y = Overall_Mean_DyadDistance + SD_DyadDistance + 0.3), 
            size = 4, hjust = 0.5, color = "black") +  # Add SD values above the bars
  theme_minimal() +
  labs(x = "Dyad", y = "Overall Mean Dyad Distance", title = "Overall Mean Dyad Distance with Error Bars", fill = "Dyad") +
...
faceted_monthly_plot_with_lines <- ggplot(data = monthly_stats, aes(x = Month, y = Mean_DyadDistance, color = Dyad, group = Dyad)) +
  ...
  geom_hline(data = grand_means, aes(yintercept = Grand_Mean_DyadDistance, color = Dyad), linetype = "dashed", size = 1) +  # Thicker dashed line for the grand mean
  facet_wrap(~ Dyad, ncol = 2) +  # Arrange facets in 2 columns for better use of space
  ...
  theme(
    strip.text = element_text(size = 14, face = "bold"),  # Increase and bold facet labels
...
# Step 1: Validate the data after filtering
filtered_data <- BexClean %>%
    filter(DyadResponse %in% c("Male aggress female", "Female aggress male"))

# Ensure data is non-empty and valid
if (nrow(filtered_data) > 0) {
    # Step 2: Group and summarize the filtered data
    overall_aggression <- filtered_data %>%
        group_by(DyadResponse) %>%
        summarise(Count = n(), .groups = 'drop')

    # Step 3: Calculate the mean of counts (handling NAs)
    mean_agg <- mean(overall_aggression$Count, na.rm = TRUE)

    # Step 4: Create the plot
    overall_aggression_plot <- ggplot(data = overall_aggression, aes(x = DyadResponse, y = Count, fill = DyadResponse)) +
        geom_bar(stat = "identity", width = 0.5) +
        geom_text(aes(label = Count), vjust = -0.5, size = 5) +
        theme_minimal(base_size = 15) +
        labs(x = "Aggression Type", y = "Count", title = "Overall Male vs Female Aggression") +
        scale_fill_manual(values = c("Male aggress female" = "#1f77b4", "Female aggress male" = "#ff7f0e")) +
        theme(legend.position = "none") +
        geom_hline(aes(yintercept = mean_agg), linetype = "dashed", color = "gray") +
        annotate("text", x = 1.5, y = mean_agg + 3, label = paste("Mean =", round(mean_agg, 2)), color = "gray", size = 5)

    # Print the plot
    print(overall_aggression_plot)
} else {
    message("No valid data found for the specified DyadResponse types.")
}
...
```{r tests on agression audience and time, echo=FALSE}
...
# Step 1: Monthly Aggression Rate Analysis

# Prepare data for monthly aggression rate analysis
BexClean <- BexClean %>%
  mutate(Month = floor_date(Date, "month"))

monthly_aggression <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(Month, DyadResponse) %>%
  summarise(AggressionCount = n(), .groups = "drop") %>%
  mutate(MonthlyAggressionRate = AggressionCount / nrow(BexClean))

# Plot monthly trends for male and female aggression separately
ggplot(monthly_aggression, aes(x = Month, y = MonthlyAggressionRate, color = DyadResponse)) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(title = "Monthly Aggression Rate Trends", x = "Month", y = "Aggression Rate",
       color = "Aggression Type") +
  theme_minimal()

# Step 2: Audience Presence and Aggression Analysis

# Categorize audience composition (male vs. female-dominated)
BexClean <- BexClean %>%
  mutate(AudienceComposition = case_when(
    AmountAudience == 0 ~ "No Audience",
    sum(nchar(Audience) == 3) > sum(nchar(Audience) == 4) ~ "Male-Dominated",
    sum(nchar(Audience) == 4) > sum(nchar(Audience) == 3) ~ "Female-Dominated",
    TRUE ~ "Mixed"
  ))

audience_aggression <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(AudienceComposition, DyadResponse) %>%
  summarise(Count = n(), .groups = 'drop')

ggplot(audience_aggression, aes(x = AudienceComposition, y = Count, fill = DyadResponse)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  theme_minimal() +
  labs(title = "Aggression Incidents by Audience Composition",
       x = "Audience Composition", y = "Aggression Count", fill = "Aggression Type") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.7), vjust = -0.3, size = 4)

# Step 3: Kinship and Aggression Analysis

# Identify kin presence within the audience
BexClean <- BexClean %>%
  mutate(KinPresent = ifelse(grepl(paste0("^", substr(Female, 1, 1)), Audience) & nchar(Female) == 4, "Female Kin Present",
                              ifelse(grepl("Nak|Xin", Audience), "Male Kin Present", "No Kin Present")))

# Analyze kin presence per dyad for aggression incidents
kin_aggression <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(KinPresent, DyadResponse) %>%
  summarise(Count = n(), .groups = 'drop')

ggplot(kin_aggression, aes(x = KinPresent, y = Count, fill = DyadResponse)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  theme_minimal() +
  labs(title = "Aggression Incidents by Kin Presence",
       x = "Kin Presence", y = "Aggression Count", fill = "Aggression Type") +
  geom_text(aes(label = Count), position = position_dodge(width = 0.7), vjust = -0.3, size = 4)

# Step 4: Contextual Interaction Analysis

# Combined analysis of month, audience composition, and kin presence
combined_context <- BexClean %>%
  group_by(Month, AudienceComposition, KinPresent) %>%
  summarise(AggressionCount = sum(DyadResponse %in% c("Male aggress female", "Female aggress male")),
            .groups = 'drop')

ggplot(combined_context, aes(x = Month, y = AggressionCount, color = AudienceComposition, linetype = KinPresent)) +
  geom_line(size = 1.2) +
  labs(title = "Interaction Between Month, Audience, and Kin Presence on Aggression",
       x = "Month", y = "Aggression Count", color = "Audience Composition", linetype = "Kin Presence") +
  theme_minimal()

# Step 5: Per-Dyad Analysis for Kin and Aggression
dyad_kin_analysis <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(Dyad, KinPresent, DyadResponse) %>%
  summarise(Count = n(), .groups = 'drop')

ggplot(dyad_kin_analysis, aes(x = Dyad, y = Count, fill = DyadResponse)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  facet_wrap(~ KinPresent) +
  theme_minimal() +
  labs(title = "Per-Dyad Kin Presence and Aggression Analysis",
       x = "Dyad", y = "Aggression Count", fill = "Aggression Type") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_text(aes(label = Count), position = position_dodge(width = 0.7), vjust = -0.3, size = 4)

# Additional code can be added based on specific questions and further refinements.
...
```{r Audience first insghts ID, echo=FALSE}

# Step 1: Filter out rows with NA in Audience and clean the Audience data
BexClean_cleaned <- BexClean %>%
  filter(!is.na(Audience)) %>%
  mutate(Audience = ifelse(Audience == "NA", "No audience", Audience)) %>%
  separate_rows(Audience, sep = "[;,.]") %>%
  mutate(Audience = trimws(Audience)) %>% # Remove leading/trailing whitespace
  filter(!grepl("BB|Knee scab|Round lips|White dot|PlainJane", Audience)) # Remove unwanted names

# Step 2: Recalculate the AmountAudience variable
BexClean_cleaned <- BexClean_cleaned %>%
  group_by(Date, Dyad) %>%
  mutate(AmountAudience = n_distinct(Audience)) %>%
  ungroup()

# Step 3: Count the number of times each individual appears in the Audience column
audience_count <- BexClean_cleaned %>%
  group_by(Audience) %>%
  summarise(Count = n()) %>%
  arrange(desc(Count))

# Reclassify UnkAM and UnkAF as Male and Female, respectively
audience_count <- audience_count %>%
  mutate(Category = case_when(
    nchar(Audience) == 3 ~ "Male",
    nchar(Audience) == 4 ~ "Female",
    Audience == "No audience" ~ "No audience",
    grepl("unkam|UnkAM", Audience, ignore.case = TRUE) ~ "Male",
    grepl("unkaf|UnkAF", Audience, ignore.case = TRUE) ~ "Female",
    grepl("unkj", Audience, ignore.case = TRUE) ~ "Unknown Juvenile",
    TRUE ~ "Unidentified"
  )) %>%
  filter(Category %in% c("Male", "Female", "No audience", "Unknown Juvenile", "Unidentified"))


# Step 4: Create and display tables for Male and Female audience members

# Male Audience Members - Top 5
top_male_count <- audience_count %>%
  filter(Category == "Male") %>%
  arrange(desc(Count)) %>%
  select(Audience, Count) %>%
  head(5)

# Display Top 5 Male Audience Members Table
cat("\n**Table: Top 5 Male Audience Members**\n")
kable(top_male_count, col.names = c("Audience Member", "Count"), align = c("l", "c"), caption = "Top 5 Male Audience Members")

# Female Audience Members - Top 5
top_female_count <- audience_count %>%
  filter(Category == "Female") %>%
  arrange(desc(Count)) %>%
  select(Audience, Count) %>%
  head(5)

# Display Top 5 Female Audience Members Table
cat("\n**Table: Top 5 Female Audience Members**\n")
kable(top_female_count, col.names = c("Audience Member", "Count"), align = c("l", "c"), caption = "Top 5 Female Audience Members")

# List of all unique Male Audience Members (without counts)
male_list <- audience_count %>%
  filter(Category == "Male") %>%
  select(Audience) %>%
  distinct()

cat("\n**List: All Male Audience Members**\n")
cat(paste(male_list$Audience, collapse = ", "))

# List of all unique Female Audience Members (without counts)
female_list <- audience_count %>%
  filter(Category == "Female") %>%
  select(Audience) %>%
  distinct()

cat("\n**List: All Female Audience Members**\n")
cat(paste(female_list$Audience, collapse = ", "))



# Step 5: Create a refined bar plot for male vs female presence in the audience including special cases
gender_presence_plot <- audience_count %>%
  ...
  theme_minimal() +
  labs(x = "Gender", y = "Total Count", title = "Male vs Female Presence in Audience") +
  scale_fill_manual(values = c("Male" = "blue", "Female" = "pink"))

# Display the plot
print(gender_presence_plot)

# Step 6: Plotting the Top 10 Audience Members
top_10_audience <- audience_count %>% 
  filter(Category %in% c("Male", "Female")) %>%
  top_n(10, Count)

top_10_plot <- ggplot(top_10_audience, aes(x = reorder(Audience, -Count), y = Count, fill = Category)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = Count), vjust = -0.5, size = 4) +
  theme_minimal() +
  labs(x = "Audience Member", y = "Count", title = "Top 10 Audience Members") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("Male" = "blue", "Female" = "pink"))

# Display the Top 10 Audience plot
print(top_10_plot)
...
```{r audience ID per Dyad? test , echo=FALSE}
# Step 7: Create tables for each dyad showing the top 10 audience members

# Function to generate and display tables for each dyad
generate_dyad_tables <- function(data, dyad_name) {
  dyad_data <- data %>%
    filter(Dyad == dyad_name) %>%
    count(Audience) %>%
    arrange(desc(n)) %>%
    top_n(10, n)
  
  cat("\n### Top 10 Audience Members for Dyad:", dyad_name, "\n")
  print(knitr::kable(dyad_data, col.names = c("Audience Member", "Count"), format = "markdown"))
}

# List of unique dyads
dyads <- unique(BexClean_cleaned$Dyad)

# Generate tables for all dyads
for (dyad in dyads) {
  generate_dyad_tables(BexClean_cleaned, dyad)
}
...
```{r Intruder IDs, echo=FALSE}

# Extract the IntruderID column
intruder_data <- BexClean$IntruderID

# Split the names by semicolons and unlist them into a single vector
intruder_names <- unlist(strsplit(intruder_data, ";"))

# Trim any leading or trailing white spaces
intruder_names <- trimws(intruder_names)

# Create a frequency table of intruder names, excluding "No Intrusion"
intruder_name_freq <- as.data.frame(table(intruder_names))
colnames(intruder_name_freq) <- c("Name", "Count")
intruder_name_freq <- intruder_name_freq[intruder_name_freq$Name != "No Intrusion", ]

# Identify 3-letter names for males
male_names <- grep("^\\w{3}$", intruder_names, value = TRUE)

# Identify 4-letter names for females
female_names <- grep("^\\w{4}$", intruder_names, value = TRUE)

# Create a new column in the frequency table indicating gender
intruder_name_freq <- intruder_name_freq %>% 
  mutate(Gender = case_when(Name %in% male_names ~ "Male",
                            Name %in% female_names ~ "Female"))

# Plot 1: Amount of Intrusions - Male vs. Female
gender_summary <- intruder_name_freq %>% 
  group_by(Gender) %>% 
  summarise(TotalIntrusions = sum(Count))

ggplot(gender_summary, aes(x = Gender, y = TotalIntrusions, fill = Gender)) +
  geom_bar(stat = "identity") +
  labs(title = "Total Intrusions by Gender", x = "Gender", y = "Total Intrusions") +
  theme_minimal()

# Plot 2: All Intruder Names Ordered by Frequency with Gender Indication
ggplot(intruder_name_freq %>% arrange(desc(Count)), 
       aes(x = reorder(Name, -Count), y = Count, fill = Gender)) +
  geom_bar(stat = "identity") +
  labs(title = "Intrusions by Individual (Ordered by Frequency)", x = "Individual", y = "Number of Intrusions") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

# Plot 3: Intrusion vs. No Intrusion
intrusion_summary <- data.frame(
  Category = c("Intrusion", "No Intrusion"),
  Count = c(nrow(intruder_name_freq), sum(intruder_data == "No Intrusion"))
)

ggplot(intrusion_summary, aes(x = Category, y = Count, fill = Category)) +
  geom_bar(stat = "identity") +
  labs(title = "Intrusion vs. No Intrusion", x = "Category", y = "Count") +
  theme_minimal()

# Display the ordered unique intruder names
print("Ordered Unique Intruder Names:")
print(sort(unique(intruder_names[intruder_names != "No Intrusion"])))

# Display the top 10 most frequent intruder names and their counts
print("Top 10 Most Frequent Intruder Names:")
print(head(arrange(intruder_name_freq, desc(Count)), 10))

# Display male names
print("Male Names (3 letters):")
print(unique(male_names))

# Display female names
print("Female Names (4 letters):")
print(unique(female_names))
...
```{r Intermediate check of not approaching, echo=FALSE}
...
# Helper function to categorize the individual by gender
categorize_individual <- function(individual_ids) {
  ids <- unlist(strsplit(individual_ids, "; "))
  genders <- ifelse(nchar(ids) == 3, "Male", ifelse(nchar(ids) == 4, "Female", NA))
  return(genders)
}

# Apply the function to create a Gender column based on ID Individual 1
BexClean_expanded <- BexClean %>%
  mutate(Gender = sapply(`IDIndividual1`, categorize_individual)) %>%
  unnest(Gender)
# Filter for "Not Approaching" occurrences
not_approaching_data <- BexClean_expanded %>%
  filter(DyadResponse == "Not Approaching")

# Count occurrences of "Not Approaching" for each gender
not_approaching_counts <- not_approaching_data %>%
  group_by(Gender) %>%
  summarize(Count = n()) %>%
  filter(!is.na(Gender))  # Filter out any NA values

# Generate the bar plot
gender_plot <- ggplot(not_approaching_counts, aes(x = Gender, y = Count, fill = Gender)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Not Approaching Occurrences by Gender",
       x = "Gender",
       y = "Count",
       fill = "Gender")
...
```{r Intermediate check of intrusion, echo=FALSE}

# Add Gender variable based on IDIndividual1 or IntruderID
BexClean <- BexClean %>%
  mutate(Gender = ifelse(nchar(IDIndividual1) == 3, "Male", 
                         ifelse(nchar(IDIndividual1) == 4, "Female", NA)))

# You can now filter and analyze based on this new Gender variable.


library(ggplot2)
library(dplyr)

# Assuming you've created the Gender variable as above

# Filter out "No Intrusion" cases
filtered_data <- BexClean %>%
  filter(IntruderID != "No Intrusion" & !is.na(Gender))

# Overall Intrusion Occurrences by Gender
intrusion_counts <- filtered_data %>%
  group_by(Gender) %>%
  summarize(Count = n())

# Plotting Overall Intrusion Occurrences by Gender
intrusion_plot <- ggplot(intrusion_counts, aes(x = Gender, y = Count, fill = Gender)) +
  geom_bar(stat = "identity", width = 0.7) +
  geom_text(aes(label = Count), vjust = -0.5) +
  geom_hline(aes(yintercept = mean(Count)), linetype = "dashed", color = "black") +
  scale_fill_manual(values = c("Male" = "#1f77b4", "Female" = "#ff7f0e")) +
  labs(title = "Overall Intrusion Occurrences by Gender",
       x = "Gender", y = "Count", fill = "Gender") +
  theme_minimal()

print(intrusion_plot)

# Intrusion Occurrences per Dyad by Gender
dyad_counts <- filtered_data %>%
  group_by(Dyad, Gender) %>%
  summarize(Count = n())

# Plotting Intrusion Occurrences per Dyad by Gender
dyad_plot <- ggplot(dyad_counts, aes(x = Dyad, y = Count, fill = Gender)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7)) +
  geom_text(aes(label = Count), vjust = -0.5, position = position_dodge(0.7)) +
  geom_hline(aes(yintercept = mean(Count)), linetype = "dashed", color = "black") +
  scale_fill_manual(values = c("Male" = "#1f77b4", "Female" = "#ff7f0e")) +
  labs(title = "Intrusion Occurrences per Dyad by Gender",
       x = "Dyad", y = "Count", fill = "Gender") +
  theme_minimal()
...
Behaviours tests
```{r behaviours graphs tests, echo=FALSE}
...
# Ensure Date is correctly formatted and clean the dataset
BexClean <- BexClean %>%
  mutate(Date = as.Date(Date, format = "%Y-%m-%d"),
         Month = floor_date(Date, "month")) %>%
  filter(DyadDistance >= 0)  # Ensure no negative distances

# Graph 1: Boxplot of Tolerance Mean Distance Per Dyad
tolerance_boxplot <- BexClean %>%
  filter(DyadResponse == "Tolerance") %>%
  group_by(Dyad, Month) %>%
  summarise(MeanDistance = mean(DyadDistance, na.rm = TRUE)) %>%
  ungroup()

ggplot(tolerance_boxplot, aes(x = Dyad, y = MeanDistance, fill = Dyad)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Mean Distance for Tolerance per Dyad",
       x = "Dyad",
       y = "Mean Distance (Tolerance)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Graph 2: Line Plot of Tolerance Distance Over Time by Dyad
ggplot(tolerance_boxplot, aes(x = Month, y = MeanDistance, color = Dyad, group = Dyad)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  theme_minimal() +
  labs(title = "Tolerance Distance Trends Over Time by Dyad",
       x = "Month",
       y = "Mean Distance (Tolerance)") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r test organization export rds and xlsx, echo=FALSE}
...
# Export BexClean as an XLSX file
write_xlsx(BexClean, path = "BexClean.xlsx")


# Export BexClean as an RDS file
saveRDS(BexClean, file = "BexClean.rds")
...
```{r Female Age, echo=FALSE}

# Load necessary libraries
library(dplyr)
library(lubridate)
library(ggplot2)  # Moved here to ensure it's loaded before use

# Step 1: Load the Life History Data
...
# Step 2: Convert FirstRecorded from character to date (dd/mm/yyyy format)
LHdata <- LHdata %>%
  mutate(
    FirstRecorded = as.Date(FirstRecorded, format = "%d/%m/%Y"),  # Convert to Date from dd/mm/yyyy
    DOB = parse_date_time(DOB, orders = c("dmy", "Y", "ymd"), quiet = TRUE)  # Parse DOB in multiple formats
  )

# Step 3: Filter female individuals
females <- c("Ndaw", "Ginq", "Xian", "Oort", "Sirk", "Piep", "Ouli", "Oerw")
LHdata_females <- LHdata %>%
  filter(Code %in% females)

# Step 4: Adjust Ouli's DOB by subtracting 4 years
LHdata_females <- LHdata_females %>%
  mutate(
    AdjustedDOB = ifelse(Code == "Ouli", as.character(FirstRecorded - years(4)), as.character(DOB)),
    AdjustedDOB = ymd(AdjustedDOB)
  )

# Step 5: Calculate Age based on AdjustedDOB
LHdata_females <- LHdata_females %>%
  mutate(
    Age = as.numeric(difftime(Sys.Date(), AdjustedDOB, units = "weeks")) / 52.1775  # Age in years
  )

# Step 6: Display the table with relevant columns
LHdata_females_table <- LHdata_females %>%
  select(Code, DOB, FirstRecorded, AdjustedDOB, Age)

# Print the table
print(LHdata_females_table)

# Step 7: Plot the Ages, sorting the bars from youngest to oldest
ggplot(LHdata_females, aes(x = reorder(Code, Age), y = Age)) + 
  geom_bar(stat = "identity", fill = "lightblue") +
  theme_minimal() +
  labs(title = "Age of Female Vervets (Sorted)", x = "Individual", y = "Age (Years)") +
  geom_text(aes(label = round(Age, 2)), vjust = -0.5)
...
```{r Male Age, echo=FALSE}


# Step 1: Filter male individuals from the Life History Data
males <- c("Sho", "Buk", "Kom", "Nge", "Xia", "Xin", "Sey", "Pom")
LHdata_males <- LHdata %>%
  filter(Code %in% males)

# Step 2: Adjust DOB for males; if DOB is missing, subtract 5 years from FirstRecorded
LHdata_males <- LHdata_males %>%
  mutate(
    AdjustedDOB = ifelse(is.na(DOB), as.character(FirstRecorded - years(5)), as.character(DOB)),
    AdjustedDOB = ymd(AdjustedDOB)
  )

# Step 3: Calculate Age based on AdjustedDOB
LHdata_males <- LHdata_males %>%
  mutate(
    Age = as.numeric(difftime(Sys.Date(), AdjustedDOB, units = "weeks")) / 52.1775
  )

# Step 4: Display the table with relevant columns
LHdata_males_table <- LHdata_males %>%
  select(Code, DOB, FirstRecorded, AdjustedDOB, Age)

# Print the table
print(LHdata_males_table)

# Step 5: Plot the Ages, sorting the bars from youngest to oldest
ggplot(LHdata_males, aes(x = reorder(Code, Age), y = Age)) + 
  geom_bar(stat = "identity", fill = "lightgreen") +
  theme_minimal() +
  labs(title = "Age of Male Vervets (Sorted)", x = "Individual", y = "Age (Years)") +
  geom_text(aes(label = round(Age, 2)), vjust = -0.5)
...
```{r Age in male and female, echo=FALSE}
# Combine male and female data
combined_data <- bind_rows(
  LHdata_females_table %>% mutate(Gender = "Female"),
  LHdata_males_table %>% mutate(Gender = "Male")
)

# Calculate the mean age across males and females
mean_age <- mean(combined_data$Age, na.rm = TRUE)

# Display the table with both male and female ages
print(combined_data)

# Plot with Male/Female distinction and mean line
ggplot(combined_data, aes(x = reorder(Code, Age), y = Age, fill = Gender)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean_age, linetype = "dashed", color = "red", size = 1) +
  theme_minimal() +
  labs(title = "Age of Male and Female Vervet Monkeys with Mean Age", x = "Individual", y = "Age (Years)") +
  scale_fill_manual(values = c("Male" = "lightblue", "Female" = "purple")) +
  geom_text(aes(label = round(Age, 2)), vjust = -0.5) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r Creation of DyadData, echo=FALSE}

# Step 1: Ensure the Dyad column is in the format "Male Female" without modifying BexClean
# Instead of modifying BexClean directly, create a new data frame
bex_dyads <- BexClean %>%
  mutate(Dyad = paste(Male, Female)) %>%
  select(Dyad, Male, Female) %>%
  distinct()

# Step 2: Define the exact valid dyads that you want
valid_dyads <- c("Sey Sirk", "Xia Piep", "Nge Oerw", "Sho Ginq", "Xin Ouli", "Buk Ndaw", "Kom Oort", "Pom Xian")

# Step 3: Filter only the valid dyads from bex_dyads
filtered_dyads <- bex_dyads %>%
  filter(Dyad %in% valid_dyads)

# Step 4: Join Male and Female age data from LHdata_males_table and LHdata_females_table
# Join Male age data
dyad_with_male_age <- filtered_dyads %>%
  left_join(LHdata_males_table %>% select(Code, Age) %>% rename(Male = Code, MaleAge = Age), by = "Male")

# Join Female age data
dyad_with_age <- dyad_with_male_age %>%
  left_join(LHdata_females_table %>% select(Code, Age) %>% rename(Female = Code, FemaleAge = Age), by = "Female")

# Step 5: Calculate the absolute age difference
dyad_with_age <- dyad_with_age %>%
  mutate(AgeDifference = abs(MaleAge - FemaleAge))

# Step 6: Filter out rows with missing values
dyad_data_clean <- dyad_with_age %>%
  filter(!is.na(MaleAge) & !is.na(FemaleAge))

# Step 7: Display the final clean data
dyad_table <- dyad_data_clean %>%
  select(Dyad, Male, Female, MaleAge, FemaleAge, AgeDifference)
...
```{r Absolute Age Difference, echo=FALSE}

# Calculate Relative Age Difference
dyad_data_clean <- dyad_data_clean %>%
  mutate(RelativeAgeDifference = (AgeDifference / ((MaleAge + FemaleAge) / 2)) * 100)

# Plot Relative Age Difference
ggplot(dyad_data_clean, aes(x = reorder(Dyad, RelativeAgeDifference), y = RelativeAgeDifference)) +
  geom_bar(stat = "identity", fill = "turquoise", width = 0.7) +
  geom_text(aes(label = paste0(round(RelativeAgeDifference, 2), "%")), vjust = -0.5, size = 4) +
  theme_minimal() +
  labs(title = "Relative Age Difference Between Dyads", x = "Dyad", y = "Relative Age Difference (%)") +
...
```{r Relative Age Difference, echo=FALSE}
...
```{r zscores age, echo=FALSE}

# Calculate Z-Score for Age Difference
dyad_data_clean <- dyad_data_clean %>%
  mutate(
    MeanAgeDiff = mean(AgeDifference, na.rm = TRUE),
    SDAgeDiff = sd(AgeDifference, na.rm = TRUE),
    ZScoreAgeDiff = (AgeDifference - MeanAgeDiff) / SDAgeDiff
  )

# Plot Z-Scores for Age Difference
ggplot(dyad_data_clean, aes(x = reorder(Dyad, ZScoreAgeDiff), y = ZScoreAgeDiff)) +
  geom_bar(stat = "identity", fill = "orange", width = 0.7) +
  geom_text(aes(label = round(ZScoreAgeDiff, 2)), vjust = -0.5, size = 4) +
  theme_minimal() +
  labs(title = "Z-Score of Age Difference Between Dyads", x = "Dyad", y = "Z-Score of Age Difference") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r Age direction, echo=FALSE}

# Create the age direction column (whether male or female is older)
dyad_data_clean <- dyad_data_clean %>%
  mutate(AgeDirection = ifelse(MaleAge > FemaleAge, "Male Older", "Female Older"))

# Simplified summary table
simplified_summary_table <- dyad_data_clean %>%
  select(Male, Female, AgeDifference, AgeDirection)

# Display the simplified summary table
print(simplified_summary_table)

# Create the bar plot visualization
ggplot(simplified_summary_table, aes(x = reorder(paste(Male, Female, sep = " - "), AgeDifference), y = AgeDifference, fill = AgeDirection)) +
  geom_bar(stat = "identity", width = 0.7) +  
  geom_text(aes(label = round(AgeDifference, 2)), vjust = -0.5, size = 4, color = "black") +
  theme_minimal() +
  scale_fill_manual(values = c("Male Older" = "#1f77b4", "Female Older" = "#ff7f0e")) +
  labs(title = "Age Difference and Age Direction Between Dyads", x = "Dyad", y = "Age Difference (Years)", fill = "Age Direction") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
...
```{r binomial tolerance, echo=FALSE}

# Load necessary libraries
library(tidyr)
library(scales)  # For percentage formatting in plots

# Step 1: Ensure that `Dyad` exists in `BexClean` without modifying existing variables
BexClean <- BexClean %>%
  mutate(Dyad = paste(Male, Female))

# Step 2: Create `ToleranceBinomial` variable in `BexClean`
BexClean <- BexClean %>%
  mutate(ToleranceBinomial = ifelse(DyadResponse == "Tolerance", 1, 0))
...
```{r tolerance distribution, echo=FALSE}


# Step 3: Create a summary table with raw values of Tolerance and No Tolerance
tolerance_summary_table <- BexClean %>%
  group_by(Dyad) %>%
  summarise(
    TotalTrials = n(),  # Count total trials per dyad
    ToleranceCount = sum(ToleranceBinomial),  # Sum of tolerance instances
    NoToleranceCount = sum(1 - ToleranceBinomial)  # Sum of no tolerance instances
  ) %>%
  mutate(
    ToleranceProportion = ToleranceCount / TotalTrials,  # Calculate tolerance proportion
    NoToleranceProportion = NoToleranceCount / TotalTrials  # Calculate no tolerance proportion
  )

# Check the summary table with proportions
print(tolerance_summary_table)



```

```{r visualization of tolerance rates, echo=FALSE}

# Step 4: Prepare data for plotting counts
tolerance_counts_long <- tolerance_summary_table %>%
  select(Dyad, ToleranceCount, NoToleranceCount) %>%
  pivot_longer(
    cols = c(ToleranceCount, NoToleranceCount),
    names_to = "ResponseType",
    values_to = "Count"
  ) %>%
  mutate(ResponseLabel = case_when(
    ResponseType == "ToleranceCount" ~ "Tolerance",
    ResponseType == "NoToleranceCount" ~ "No Tolerance"
  ))

# Step 5: Prepare data for plotting proportions
tolerance_proportions_long <- tolerance_summary_table %>%
  select(Dyad, ToleranceProportion, NoToleranceProportion) %>%
  pivot_longer(
    cols = c(ToleranceProportion, NoToleranceProportion),
    names_to = "ResponseType",
    values_to = "Proportion"
  ) %>%
  mutate(ResponseLabel = case_when(
    ResponseType == "ToleranceProportion" ~ "Tolerance",
    ResponseType == "NoToleranceProportion" ~ "No Tolerance"
  ))


# Step 6: Order dyads by ToleranceProportion
tolerance_summary_table <- tolerance_summary_table %>%
  arrange(ToleranceProportion) %>%
  mutate(Dyad = factor(Dyad, levels = Dyad))

# Apply the same ordering to the long data frames
tolerance_counts_long <- tolerance_counts_long %>%
  mutate(Dyad = factor(Dyad, levels = levels(tolerance_summary_table$Dyad)))

tolerance_proportions_long <- tolerance_proportions_long %>%
  mutate(Dyad = factor(Dyad, levels = levels(tolerance_summary_table$Dyad)))


# Step 7: Plot counts of Tolerance and No Tolerance per Dyad
ggplot(tolerance_counts_long, aes(x = Dyad, y = Count, fill = ResponseLabel)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = Count), position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  labs(title = "Counts of Tolerance and No Tolerance per Dyad",
       x = "Dyad", y = "Count", fill = "Response") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Step 8: Plot proportions of Tolerance and No Tolerance per Dyad
ggplot(tolerance_proportions_long, aes(x = Dyad, y = Proportion, fill = ResponseLabel)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = scales::percent(Proportion, accuracy = 0.1)),
            position = position_stack(vjust = 0.5), size = 3) +
  labs(title = "Proportion of Tolerance vs. No Tolerance per Dyad",
       x = "Dyad", y = "Proportion", fill = "Response") +
...
```{r add age difference in Dyad, echo=FALSE}

# Step 9: Merge tolerance summary with dyad age data
dyad_summary <- dyad_data_clean %>%
  select(Dyad, Male, Female, AgeDifference, AgeDirection) %>%  # Ensure these columns are included
  left_join(tolerance_summary_table, by = "Dyad")
...
```{r Absolute age difference & tolerance, echo=FALSE}

# Assuming your data frame is named 'dyad_summary' and has the columns 'AgeDifference' and 'ToleranceProportion'

# Scatter plot with regression line
library(ggplot2)

ggplot(dyad_summary, aes(x = AgeDifference, y = ToleranceProportion)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Tolerance Proportion vs. Age Difference",
       x = "Age Difference (years)",
       y = "Tolerance Proportion") +
  theme_minimal()

# Correlation test
cor_test <- cor.test(dyad_summary$AgeDifference, dyad_summary$ToleranceProportion)
print(cor_test)

# Linear regression
model <- lm(ToleranceProportion ~ AgeDifference, data = dyad_summary)
summary(model)
...
Earlier Pearson's Correlation:
Correlation Coefficient (r): Approximately 0.0784
P-value: 0.8535
Interpretation: No significant linear relationship.
Spearman's Correlation:
Correlation Coefficient (rho): 0.4286
P-value: 0.2992
Interpretation: Suggests a moderate monotonic relationship but not statistically significant.
Why the Difference?
...
Less sensitive to outliers and does not require normal distribution.

```{r Age Spearman, echo=FALSE}

# Spearman's correlation test
spearman_test <- cor.test(dyad_summary$AgeDifference, dyad_summary$ToleranceProportion, method = "spearman")
print(spearman_test)

# Identify potential outliers
library(ggpubr)
ggboxplot(dyad_summary, x = "Dyad", y = "ToleranceProportion", add = "jitter")


# Plotting ranked data
ggplot(dyad_summary, aes(x = rank(AgeDifference), y = rank(ToleranceProportion))) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(
    title = "Ranked Tolerance Proportion vs. Ranked Age Difference",
    x = "Rank of Age Difference",
    y = "Rank of Tolerance Proportion"
  ) +
  theme_minimal()
...
```{r Age direction & Tolerance, echo=FALSE}


# Step 10: Plot Tolerance Proportion vs. Age Difference
ggplot(dyad_summary, aes(x = AgeDifference, y = ToleranceProportion)) +
  geom_point(size = 3, color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "darkblue") +
  labs(title = "Tolerance Proportion vs. Age Difference",
       x = "Age Difference (Years)", y = "Tolerance Proportion") +
  theme_minimal()


# Step 11: Plot Tolerance Proportion by Age Direction
ggplot(dyad_summary, aes(x = AgeDirection, y = ToleranceProportion, fill = AgeDirection)) +
  geom_boxplot(alpha = 0.7) +
  geom_jitter(width = 0.1, size = 3, color = "black") +
  labs(title = "Tolerance Proportion by Age Direction",
       x = "Age Direction", y = "Tolerance Proportion") +
  scale_fill_manual(values = c("Male Older" = "#1f77b4", "Female Older" = "#ff7f0e")) +
  theme_minimal() +
  theme(legend.position = "none")


# Step 13: T-test for Tolerance Proportion by Age Direction
t_test_result <- t.test(ToleranceProportion ~ AgeDirection, data = dyad_summary)
print(t_test_result)
...
```{r ABsolute age + Age direction & Tolerance, echo=FALSE}

#Regression model 

# Fit the model
model3 <- lm(ToleranceProportion ~ AgeDifference + AgeDirection, data = dyad_summary)
...
ggplot(dyad_summary, aes(x = AgeDifference, y = ToleranceProportion, color = AgeDirection)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Tolerance Proportion vs. Age Difference by Age Direction",
       x = "Age Difference (Years)",
       y = "Tolerance Proportion",
       color = "Age Direction") +
  theme_minimal()
...
model3_interaction <- lm(ToleranceProportion ~ AgeDifference * AgeDirection, data = dyad_summary)

# Summarize the model
summary(model3_interaction)


# Scatter plot with regression lines for each AgeDirection
ggplot(dyad_summary, aes(x = AgeDifference, y = ToleranceProportion, color = AgeDirection)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(title = "Tolerance Proportion vs. Age Difference by Age Direction",
...
* Check of model assumptiuons: Residuals vs. Fitted: Check for linearity and homoscedasticity.
...
# Format date columns to proper date format
...
LHdata <- subset(LHdata, !is.na(LHdata$Code))

# Create Male and Female columns based on the length of the 'Code' (Male = 3 letters, Female = 4 letters)
LHdata$Male <- ifelse(nchar(LHdata$Code) == 3, LHdata$Code, NA)
LHdata$Female <- ifelse(nchar(LHdata$Code) == 4, LHdata$Code, NA)

# Create Sex column based on whether the individual is classified as Male or Female
LHdata$Sex <- ifelse(!is.na(LHdata$Male), "M", ifelse(!is.na(LHdata$Female), "F", NA))

# Retain necessary columns, including columns 10 to 26
LHdata <- LHdata[, c("Code", "Sex", "Male", "Female", "DOB", "FirstRecorded", "DateImmigration1", 
                     "DepartureNatalGp", "PresentGp", "BirthGp", "Mother", "Father", 
                     "ImmigrationGp1", "DateImmigration1", "LastSeen1", "ImmigrationGp2", 
                     "DateImmigration2", "LastSeen2", "ImmigrationGp3", "DateImmigration3", 
                     "LastSeen3", "ImmigrationGp4", "DateImmigration4", "LastSeen4", 
                     "ReliableData")]

# Function to calculate age based on different dates
calculate_age <- function(date_of_birth, first_recorded, departure_date, target_date, departure_natal) {
  if (is.na(date_of_birth)) {
    ...
    if (!is.na(departure_date) && departure_date <= target_date) {
      if(is.na(departure_natal)){
        age_in_years <- age_in_years + 5  # Adding 5 years in case departure_natal is not available
      } 
    }
  } else {
...
# Calculate age for each individual in the dataset
LHdata <- LHdata %>%
  rowwise() %>%
  mutate(Age = calculate_age(DOB, FirstRecorded, DepartureNatalGp, Sys.Date(), ReliableData))  # Using Sys.Date() as a proxy for current date

# Revised function to classify individuals based on their age and sex
get_age_class <- function(Sex, Age, departure_natal, target_date) {
  Adjuv <- vector("character", length = length(Sex))  # Preallocate a vector for results
  for (i in seq_along(Sex)) {
    if (is.na(Sex[i]) || is.na(Age[i])) {
      Adjuv[i] <- NA
    } else {
      if (Sex[i] == "F" && Age[i] >= 4) {
        Adjuv[i] <- "AF"  # Adult Female
      } else if (Sex[i] == "F" && Age[i] >= 1 && Age[i] < 4) {
        Adjuv[i] <- "JF"  # Juvenile Female
      } else if (Sex[i] == "M" && Age[i] >= 1 && Age[i] < 4) {
        Adjuv[i] <- "JM"  # Juvenile Male
      } else if (Sex[i] == "M" && Age[i] >= 4 && Age[i] < 5 && (!is.na(departure_natal[i]) && departure_natal[i] > target_date[i])) {
        Adjuv[i] <- "SM"  # Subadult Male
      } else if (Sex[i] == "M" && Age[i] >= 4 && Age[i] < 5 && (!is.na(departure_natal[i]) && departure_natal[i] < target_date[i])) {
        Adjuv[i] <- "AM"  # Adult Male
...
# Apply the age class function to the dataset
LHdata$AgeClass <- get_age_class(LHdata$Sex, LHdata$Age, LHdata$DepartureNatalGp, Sys.Date())  # Using Sys.Date() as target date

# Final result - View or print the final dataframe
View(LHdata)
...
* Speciffically for these codes I may have seqcheck() and elo.seq() functions errors as:
	-First interaction occurred before presence (approx) meaning that there's already an interaction recorded before the male was added into that group in the life history
...
```{r Final Agonistic, echo=FALSE}

## Creating FinalAgonistic
library(dplyr)
library(tidyr)
library(stringr)
...
new <- read.csv("/Users/maki/Desktop/Master Thesis/Josies Codes/Agonistic.csv",1)

# We only use one on one interactions
# So omit all interactions with a supporter

new <- dplyr::filter(new, IDSupporters == "")
...
new$Time <- format(as.POSIXct(new$Time,format="%H:%M:%S"),"%H:%M:%S")
colnames(new)[1] <- "Date"
new$Date <- as.Date(format(as.POSIXct(new$Date, format = "%Y/%m/%d"), "%Y-%m-%d"))
colnames(new)[4] <- "Aggressor"
colnames(new)[5] <- "AggressorBehaviour"
colnames(new)[6] <- "Victim"
colnames(new)[7] <- "VictimBehaviour"
str(new)

# Combine the old and new dataframes:
elo <- bind_rows(old, new)
#View(elo)

# Make sure there is no overlapping data with the old data
elo <- elo %>% distinct()
...
f <- read.csv("/Users/maki/Desktop/Master Thesis/Josies Codes/Focal.CSV",1)
f$Date <- as.Date(format(as.POSIXct(f$Date, format = "%Y/%m/%d"), "%Y-%m-%d"))

# Filter only agonistic without support
f <- subset(f, f$Behaviour%in% "Agonistic") 
f <- subset(f, !f$AgonisticInteraction%in% c("Support to Focal", "Support to Partner", "Focal Supports"))
...
f <- f %>%
  mutate(VictimBehaviour = str_extract_all(BehaviourFocal, "(?<=\\b[bB])(\\w{2,4})")) %>%
  mutate(VictimBehaviour = ifelse(lengths(VictimBehaviour) > 0, sapply(VictimBehaviour, function(x) paste(x, collapse = " ")), NA),
         BehaviourFocal = str_replace_all(BehaviourFocal, "\\b[bB]\\w{2,4}\\b", ""))

colnames(f) [1:7] <- c("Date", "Time", "Group", "Aggressor", "AggressorBehaviour", "Victim", "VictimBehaviour")


# we bind both focals together
focals <- bind_rows(foc_old, f)

# Make sure there's no duplicates
focals <- focals %>% distinct()
...
# Save this data frame as FinalAgonistic
write.csv(final, file = "/Users/maki/Desktop/Master Thesis/Josies Codes/FinalAgonistic.csv", row.names = FALSE)




# However, although the column names are Aggressor and Victim, we do not know yet which individual won the interaction, and which one lost.
...
### 6.2 Creation of Winner Looser


```{r Winner Looser, echo=FALSE}
...
# Load FinalAgonistic data created by the "Create FinalAgonistic.R" script
...
missing_dates <- d[is.na(d$Date), ]

if (nrow(missing_dates) > 0) {
  print("Missing dates found in FinalAgonistic data:")
  print(missing_dates)  # Print rows with missing dates for review
  
  # Remove rows with missing dates in the 'Date' column
  d <- d[!is.na(d$Date), ]
  
  # Check after removing missing dates
  print("After removing missing dates:")
  print(head(d))
} else {
  print("No missing dates found in FinalAgonistic data.")
}

# Remove any remaining rows with missing data (just in case)
d <- na.omit(d)

# Ensure 'Date' is properly formatted again
d$Date <- as.Date(d$Date, format = "%Y-%m-%d")

# Remove rows with dates before 2020 (as per your timeframe)
d <- d[d$Date >= "2020-01-01", ]

# Sort data by Date to prepare for Elo ratings
d <- dplyr::arrange(d, Date)

# Convert behavior descriptions to lowercase for consistency
d$AggressorBehaviour <- tolower(d$AggressorBehaviour)
d$VictimBehaviour <- tolower(d$VictimBehaviour)

#### Winner/Loser Determination ####

# Define submissive behaviors for victims and aggressive behaviors for aggressors
ret_beh <- c('fl', 'rt', 'av', 'ja', 'cr', 'ss', 'gu')  # Submissive behaviors
agg_cat <- list(
  cat_3 = c('bi', 'gb', 'hi', 'fi', 'hh', 'so'),  # Severe aggressive behaviors
  cat_2 = 'ch',  # Chase behavior
  cat_1 = c('ac', 'at', 'dp', 'tp', 'st', 'su', 'fh', 'sf', 'hb', 'bd')  # Mild aggressive behaviors
)

# Function to determine the winner based on behaviors
decide.win <- function(beh_x, beh_y) {
  x <- 0
  y <- 0
  
  # Count submissive behaviors
  for (beh_ in ret_beh) {
  ...
  return(0)  # If no clear winner
}

# Apply the function to determine winners and losers
d$win_lose <- mapply(decide.win, d$AggressorBehaviour, d$VictimBehaviour)

# Prepare the final dataset for Elo rating analysis
d$winner <- ifelse(d$win_lose == 1, d$Aggressor, d$Victim)
d$loser <- ifelse(d$win_lose == 1, d$Victim, d$Aggressor)

# Mark interactions without a clear winner as a draw
d$Draw <- ifelse(d$win_lose == 0, TRUE, FALSE)

# Add intensity levels to interactions based on behaviors
d$intensity <- NA
mild <- grepl("ap|ac|ag|at|dp|tp|st|vo|gu|sc|hb", d$AggressorBehaviour)
d[mild, "intensity"] <- "mild"
severe <- grepl("bi|gb|hi|fi|hh|so", d$AggressorBehaviour)
d[severe, "intensity"] <- "severe"

# Final check: Print the first few rows of the data to ensure correctness
print(head(d))

# Save the final dataset for Elo rating
write.csv(d, "/Users/maki/Desktop/Master Thesis/Josies Codes/WinnerLoser.csv", row.names = FALSE)

cat("WinnerLoser.csv successfully created with", nrow(d), "rows.\n")
...
*   **BD1		>	SEPT 2022**
      a. Sey Sirk	:	14.09.2022
      b. Xia Piep	:	16.09.2022
      c. Nge Oerw	:	22.09.2022
      d. Xin Ouli	:	27.09.2022

*   **AK		>	SEPT 2022**
      e. Sho Ginq	:	27.09.2022
      f. Buk Ndaw	:	29.09.2022
      
*   **BD2		>	DEC 2022**
      h. Kom Oort	:	12.12.2022
      
*   **NH		>	MAR 2023**
      i. Pom Xian	:	10.03.2023
...
* 1. BD1, Will calculate elo for all females in BD group so we can extract the ones of **Sirk, Piep, Oerw, Ouli** while for males we will look at the rank of **Sey, Xia, Nge, Xin**I will calculate their elo using the presence data and winner looser data until the **13.10.2022** 
  ...
  4. Last Elo calculations for males and females in NH will use data until the **09.03.23** to extract rank of female **Xian** and Male**Pom**
### 6.2 Female Elo Ratings
...
BDpres <- BDpres[!is.na(BDpres$Date), ]
...
NHpres <- NHpres[!is.na(NHpres$Date), ]

# Load interaction data (WinnerLoser)
...
# Debugging: Identify rows with missing dates
if (any(is.na(d$Date))) {
  missing_dates <- d[is.na(d$Date), ]
  print("Rows with missing dates in interaction data:")
  print(missing_dates)
  d_clean <- d[!is.na(d$Date), ]  # Remove rows with missing dates
} else {
  d_clean <- d
}

# Step 1: Identify Date Ranges for presence data and fix list printing
presence_dates <- list(
  AK = range(AKpres$Date, na.rm = TRUE),
  BD = range(BDpres$Date, na.rm = TRUE),
  NH = range(NHpres$Date, na.rm = TRUE)
)

interaction_dates <- range(d_clean$Date, na.rm = TRUE)

# Print the presence data ranges properly
cat("Presence Data Ranges:\n")
for (group in names(presence_dates)) {
  cat(group, ": ", presence_dates[[group]], "\n")
}

cat("Interaction Data Range:", interaction_dates, "\n")

# Step 2: Identify Mismatched IDs
# Assuming that presence files have an 'ID' column for individuals' IDs

presence_ids <- unique(c(AKpres$ID, BDpres$ID, NHpres$ID))  # Combine presence IDs
interaction_ids <- unique(c(d_clean$winner, d_clean$loser))

missing_in_presence <- setdiff(interaction_ids, presence_ids)
missing_in_interaction <- setdiff(presence_ids, interaction_ids)

cat("IDs in interaction data but not in presence data:", missing_in_presence, "\n")
cat("IDs in presence data but not in interaction data:", missing_in_interaction, "\n")

# Step 3: Filter Interaction Data to Match Presence Date Range
min_presence_date <- min(unlist(presence_dates), na.rm = TRUE)
max_presence_date <- max(unlist(presence_dates), na.rm = TRUE)

d_filtered <- d_clean[d_clean$Date >= min_presence_date & d_clean$Date <= max_presence_date, ]

cat("Filtered interaction data date range:", range(d_filtered$Date), "\n")

# Optional: Remove rows with mismatched IDs from interaction data
d_filtered <- d_filtered[d_filtered$winner %in% presence_ids & d_filtered$loser %in% presence_ids, ]

cat("Final cleaned interaction data has", nrow(d_filtered), "rows.\n")

# Proceed with seqcheck if dates and IDs are clean and aligned
seqcheck(winner = d_filtered$winner, loser = d_filtered$loser, Date = d_filtered$Date, draw = NULL, presence = AKpres)
...
pres_ak <- read.csv("/Users/maki/Desktop/Master Thesis/Josies Codes/Ankhase_presence_matrix.csv")
pres_bd <- read.csv("/Users/maki/Desktop/Master Thesis/Josies Codes/Baie Dankie_presence_matrix.csv")
pres_nh <- read.csv("/Users/maki/Desktop/Master Thesis/Josies Codes/Noha_presence_matrix.csv")
...
ak <- DSI(dsi_ak, ot.source = ot_ak, duration.NA.treatm = "count", onlyfocaldyads = F, limit2focalnonfocal = F)
bd <- DSI(dsi_bd, ot.source = ot_bd, duration.NA.treatm = "count", onlyfocaldyads = F, limit2focalnonfocal = F)
nh <- DSI(dsi_nh, ot.source = ot_nh, duration.NA.treatm = "count", onlyfocaldyads = F, limit2focalnonfocal = F)
...
```{r distance & tolerance proprotion, echo=FALSE}
...
# Remove NA values and summarize data: count tolerance and non-tolerance occurrences by distance
tolerance_summary <- BexClean %>%
  filter(!is.na(DyadDistance) & !is.na(ToleranceBinomial)) %>%
  group_by(DyadDistance, ToleranceBinomial) %>%
  summarise(Count = n())

# Create the bar chart
ggplot(tolerance_summary, aes(x = DyadDistance, y = Count, fill = factor(ToleranceBinomial))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("0" = "red", "1" = "blue"), labels = c("No Tolerance", "Tolerance")) +
  labs(x = "Distance", y = "Count of Interactions", fill = "Tolerance Type") +
  theme_minimal() +
  ggtitle("Tolerance and Non-Tolerance Across Distances")





# Create a boxplot for Distance vs Tolerance
ggplot(BexClean, aes(x = factor(ToleranceBinomial), y = DyadDistance, fill = factor(ToleranceBinomial))) +
  geom_boxplot() +
  scale_fill_manual(values = c("0" = "red", "1" = "blue"), labels = c("No Tolerance", "Tolerance")) +
  labs(x = "Tolerance Type", y = "Distance", fill = "Tolerance Type") +
  theme_minimal() +
  ggtitle("Boxplot of Distance by Tolerance")
...
```{r distance & tolerance assumpations, echo=FALSE}



# Normality test for DyadDistance
shapiro.test(BexClean$DyadDistance)

# Normality test for ToleranceBinomial
shapiro.test(BexClean$ToleranceBinomial)
...
```{r Distance & Tolerance Wilcoxon and t test, echo=FALSE}


# Perform Wilcoxon rank sum test (Mann-Whitney U test)
wilcox_test <- wilcox.test(DyadDistance ~ ToleranceBinomial, data = BexClean)
wilcox_test
...
t_test <- t.test(DyadDistance ~ ToleranceBinomial, data = BexClean)
t_test
...
tep 7: Reporting the Results
In your write-up, you would report something like this:
...
```{r Dyad and distance & tolerance, echo=FALSE}

# Summarize tolerance data per dyad and distance
tolerance_summary_per_dyad <- BexClean %>%
  filter(!is.na(DyadDistance) & !is.na(ToleranceBinomial)) %>%
  group_by(Dyad, DyadDistance, ToleranceBinomial) %>%
  summarise(Count = n())

# Bar chart for each dyad
ggplot(tolerance_summary_per_dyad, aes(x = DyadDistance, y = Count, fill = factor(ToleranceBinomial))) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~Dyad) +  # Split by dyad
  scale_fill_manual(values = c("0" = "red", "1" = "blue"), labels = c("No Tolerance", "Tolerance")) +
  labs(x = "Distance", y = "Count of Interactions", fill = "Tolerance Type") +
  theme_minimal() +
  ggtitle("Tolerance and Non-Tolerance Across Distances for Each Dyad")




# Boxplot for distance vs. tolerance for each dyad
ggplot(BexClean, aes(x = factor(ToleranceBinomial), y = DyadDistance, fill = factor(ToleranceBinomial))) +
  geom_boxplot() +
  facet_wrap(~Dyad) +  # Split by dyad
  scale_fill_manual(values = c("0" = "red", "1" = "blue"), labels = c("No Tolerance", "Tolerance")) +
  labs(x = "Tolerance Type", y = "Distance", fill = "Tolerance Type") +
  theme_minimal() +
  ggtitle("Boxplot of Distance by Tolerance for Each Dyad")
...
```{r distance dyad & tolerance test test, echo=FALSE}
...
# Check normality for DyadDistance using Shapiro-Wilk test
shapiro.test(BexClean$DyadDistance)

# Check homogeneity of variance (Levene's Test) - optional
leveneTest(DyadDistance ~ factor(ToleranceBinomial), data = BexClean)



# Logistic regression model: Tolerance explained by Distance and Dyad
logistic_model <- glm(ToleranceBinomial ~ DyadDistance * Dyad, data = BexClean, family = "binomial")
...
# Loop through each dyad and run the Wilcoxon test
results <- BexClean %>%
  filter(!is.na(DyadDistance) & !is.na(ToleranceBinomial)) %>%
  group_by(Dyad) %>%
  do(test = wilcox.test(DyadDistance ~ ToleranceBinomial, data = .))

# View the results for each dyad
results



# Add predicted tolerance probabilities to the dataset
BexClean$predicted_tolerance <- predict(logistic_model, type = "response")

# Plot the predicted probabilities of tolerance by distance for each dyad (Loess smoother)
ggplot(BexClean, aes(x = DyadDistance, y = predicted_tolerance, color = Dyad)) +
  geom_point() +
  geom_smooth(method = "loess", se = FALSE) +
  labs(x = "Distance", y = "Predicted Tolerance Probability", color = "Dyad") +
  theme_minimal() +
  ggtitle("Predicted Tolerance by Distance for Each Dyad (Loess Smoother)")





# Plot the predicted probabilities of tolerance by distance for each dyad (Linear smoother)
ggplot(BexClean, aes(x = DyadDistance, y = predicted_tolerance, color = Dyad)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Distance", y = "Predicted Tolerance Probability", color = "Dyad") +
  theme_minimal() +
  ggtitle("Predicted Tolerance by Distance for Each Dyad (Linear Smoother)")
...
```{r toelrance & Dyad, echo=FALSE}


# Summarize tolerance proportion by dyad
tolerance_proportion_summary <- BexClean %>%
  group_by(Dyad) %>%
  summarise(ToleranceProportion = mean(ToleranceBinomial, na.rm = TRUE))

# Run ANOVA to test if dyad explains variance in tolerance proportion
anova_model <- aov(ToleranceProportion ~ Dyad, data = tolerance_proportion_summary)
summary(anova_model)



# Kruskal-Wallis test to check if dyad explains variance in tolerance proportion
kruskal_test <- kruskal.test(ToleranceProportion ~ Dyad, data = tolerance_proportion_summary)
print(kruskal_test)
...
1. Visualizing Behavioral Trends Over Time
We'll start by analyzing how key behaviors (like tolerance, male aggression, female aggression, and not approaching) evolve over time.

Goal: Understand if there's a consistent trend over time, indicating factors like seasonality, repeated interactions, or changes in dyad dynamics.
Approach: We'll create time-series plots to visualize how these behaviors change across trials and experiment days.
2. Analyzing Behavioral Trends by Dyad
Each dyad may exhibit different patterns. By separating the trends by dyad, we can:

Goal: Identify whether some dyads are consistently tolerant, aggressive, or show variations depending on conditions like audience or time.
Approach: Create faceted plots, where each facet represents a different dyad, showing the evolution of behaviors like distance, tolerance, and aggression.
3. Investigating the Impact of Audience
Understanding how audience composition influences behaviors like tolerance or aggression is crucial.

Goal: Determine if audience types (e.g., kin vs. non-kin) have a statistically significant effect on behaviors.
Approach: Use boxplots and bar charts to show the relationship between audience composition (kin vs. non-kin) and behaviors.
4. Postulate and Refine Hypotheses
After the initial EDA, we’ll refine the hypotheses and decide on the most suitable statistical models.

Examples of Hypotheses:
Tolerance is associated with shorter dyad distances over time.
Audience presence (kin vs. non-kin) influences the likelihood of aggression.
5. Preparing for Mixed-Effects Models
Once the EDA provides insights, we can set up mixed-effects models that account for:

Repeated measures within each dyad.
Random effects to capture variability across dyads or trials.
6. Outlier Detection and Analysis
Outliers might distort the analysis, so we'll need to:

Goal: Identify and handle influential outliers that could bias our models.
Approach: Use boxplots and statistical diagnostics (e.g., Cook's distance) to detect outliers.
