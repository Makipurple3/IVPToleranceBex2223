...
* **24 values** were inserted in **Distance** to replace the NA's where the distance could be found by looking at the previous rows. The **46 remaining NA's** were then **removed** from Distance **leaving 0 NA in Distance**

#### 2.9.2 Creation of No Trial
...
# Initialize the necessary changes only if they haven't been done before
if (!"changes_applied" %in% names(Bex)) {
  Bex$changes_applied <- FALSE  # Initialize a control column
}

# Check if changes have already been applied
if (!any(Bex$changes_applied)) {
  ...
  # Initialize NoTrial variable before deleting rows
  Bex$NoTrial <- rep(0, nrow(Bex))  # Initialize all as 0
  Bex$NoTrial[rows_to_delete] <- 1  # Set to 1 for specified trials to delete

  # Deleting rows
  Bex <- Bex[-rows_to_delete, ]

  # Mark changes as applied
  Bex$changes_applied <- TRUE
}

# Output the dataset to check
print(head(Bex))
...
* Before cleaning Female and Male ID, here is a list of every dyad of the ebox experiment and their respective groups. This will help us find the missing names when only one individual is missing out of the duo (either male or female):
  a. Sirk & Sey - BD
  b. Ouli & Xin - BD
  c. Piep & Xia - BD
  d. Oerw & Nge - BD
...
* First I have to check where are the NA's in both FemaleID and Male ID by looking at the rows where data is missing. Since every trial was made with a Dyad and never with an single individual, treating these two columns together makes more sense. 
...
* In order to clean FemaleID, I will use the data from the now complete MaleID. I will use conditions stating that depending which name is found in MaleID when there is an NA in FemaleID, a certain name will have to replace the NA in female ID:
  ...
  If there is NA in female ID, we will replace the value with 
...
* After the use of the conditions in FemaleID I could see the changes where successufly done and that 0 NA's are remaing in both FemaleID and MaleID
