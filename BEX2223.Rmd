...
replace_losing_interest <- function(df) {
  ...
  initial_losing_interest_count <- sum(grepl("Losing interest", df$DyadResponse))
  initial_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))
  initial_not_approaching_duplicate_count <- sum(grepl("Not approaching; Not approaching", df$DyadResponse))
  
  # Step 1: Replace "Losing interest" with "Not approaching"
  df <- df %>%
    mutate(DyadResponse = gsub("Losing interest", "Not approaching", DyadResponse))
  
  # Count occurrences after Step 1
  post_step1_losing_interest_count <- sum(grepl("Losing interest", df$DyadResponse))
  post_step1_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))

  # Display the changes after Step 1
  cat("Step 1 - Replace 'Losing interest' with 'Not approaching':\n")
  cat("Number of 'Losing interest' occurrences before: ", initial_losing_interest_count, "\n")
  cat("Number of 'Losing interest' occurrences after: ", post_step1_losing_interest_count, "\n")
  cat("Number of 'Not approaching' occurrences before: ", initial_not_approaching_count, "\n")
  cat("Number of 'Not approaching' occurrences after: ", post_step1_not_approaching_count, "\n\n")

  # Step 2: Remove duplicate "Not approaching" entries
  df <- df %>%
    mutate(DyadResponse = gsub("Not approaching(\\s*;\\s*Not approaching)+", "Not approaching", DyadResponse))

  # Count occurrences after Step 2
  final_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))
  final_not_approaching_duplicate_count <- sum(grepl("Not approaching; Not approaching", df$DyadResponse))

  # Display the changes after Step 2
  cat("Step 2 - Remove duplicate 'Not approaching' entries:\n")
  cat("Number of 'Not approaching' duplicates before: ", initial_not_approaching_duplicate_count, "\n")
  cat("Number of 'Not approaching' duplicates after: ", final_not_approaching_duplicate_count, "\n")
  cat("Final number of 'Not approaching' occurrences: ", final_not_approaching_count, "\n")
...
print(knitr::kable(unique_dyad_response_table, col.names = c("DyadResponse", "Frequency"), caption = "Unique DyadResponse Entries and Their Counts"))
...
# Function to replace "Losing interest" with "Not approaching" and clean up the DyadResponse column
replace_losing_interest <- function(df) {
  # Step 1: Count initial occurrences
  initial_losing_interest_count <- sum(grepl("Losing interest", df$DyadResponse))
  initial_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))

  # Step 2: Replace "Losing interest" with "Not approaching"
  df <- df %>%
    mutate(DyadResponse = gsub("Losing interest", "Not approaching", DyadResponse))

  # Step 3: Remove duplicate "Not approaching" entries
  df <- df %>%
    mutate(DyadResponse = gsub("(Not approaching\\s*;\\s*)+", "Not approaching; ", DyadResponse),
           DyadResponse = gsub("Not approaching;\\s*$", "Not approaching", DyadResponse)) # Remove trailing semicolon if present

  # Step 4: Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)

  # Step 5: Count final occurrences
  final_losing_interest_count <- sum(grepl("Losing interest", df$DyadResponse))
  final_not_approaching_count <- sum(grepl("Not approaching", df$DyadResponse))

  # Display the changes
  cat("Initial 'Losing interest' count: ", initial_losing_interest_count, "\n")
  cat("Initial 'Not approaching' count: ", initial_not_approaching_count, "\n")
  cat("Final 'Losing interest' count: ", final_losing_interest_count, "\n")
  cat("Final 'Not approaching' count: ", final_not_approaching_count, "\n")
...
replace_distracted_with_not_approaching <- function(df) {
  ...
  # Return updated dataframe and counts
  return(list(df, distracted_changes, not_approaching_changes))
}

# Apply the function to the dataframe
result_replace_distracted <- replace_distracted_with_not_approaching(BexClean)

# Extract the updated dataframe and counts
BexClean <- result_replace_distracted[[1]]
distracted_changes <- result_replace_distracted[[2]]
not_approaching_changes <- result_replace_distracted[[3]]
...
extract_unique_keywords <- function(df) {
  ...
  colnames(unique_keywords_df) <- c("Keyword", "Frequency")
