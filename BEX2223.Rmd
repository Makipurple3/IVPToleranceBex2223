...
# Important chunk for dyad sumamry to keep!
...
# SELECT FINAL DATASET & EXPORT FOR MANIPULATION


* Here are the New Variables to Add
  -MAge and FAge: Male and female ages in years.
  -AgeDiff: Absolute difference between MAge and FAge.
  -IEloDiff: Absolute difference between male and female Elo quartiles.   
  -Binomial Variables (Tol, Agg, NotApp): Based on your criteria.
  -Existing variables in Bex: "Date", "Month", "DyadDay", "Trial", "Time", "Period", "Male", "Female", "Dyad", "DyadDistance", "Proximity", "DyadResponse", "SpecialBehaviour", "Context", "ToleranceBinomial"



```{r export dyadsummary csv, echo=FALSE}
# Select and rename specific columns
dyad_summary_clean <- dyad_summary %>%
  select(
    Dyad, 
    Male, 
    Female, 
    AgeDiff = AgeDifference, # Rename AgeDifference to AgeDiff
    AgeDir = AgeDirection,   # Rename AgeDirection to AgeDir
    TotalTrials,
    ToleranceCount,
    NoToleranceCount,
    ToleranceProportion,
    NoToleranceProportion
  )

print(dyad_summary)

# Export the cleaned dataset to a CSV file
write.csv(dyad_summary_clean, "/Users/maki/Desktop/Master Thesis/BEX 2223 Master Thesis Maung Kyaw/IVPToleranceBex2223/dyad_summary_clean.csv", row.names = FALSE)
```




```{R FINAL BEX FOR STATS, echo=FALSE}

# Step 1: Identify and review column names in Bex and in Dyad Summary

colnames(BexClean)
colnames(dyad_summary)
# Step 2: Reorder and select variables of interest
# Rearrange columns to bring important ones to the front


BexClean <- BexClean %>%
  rename(Male = MaleID, Female = FemaleID)



BexClean <- BexClean %>%
  select(Date, Time, Group, Male, Female, DyadResponse, )

# Step 3: Count occurrences of Tolerance, Aggression, and NotApproaching
response_counts <- table(Bex$DyadResponse)
print(response_counts)

tolerance_count <- sum(BexClean$Tol == 1, na.rm = TRUE)
aggression_count <- sum(BexClean$Agg == 1, na.rm = TRUE)
not_approaching_count <- sum(BexClean$NotApp == 1, na.rm = TRUE)

cat("Tolerance Count:", tolerance_count, "\n")
cat("Aggression Count:", aggression_count, "\n")
cat("Not Approaching Count:", not_approaching_count, "\n")


# Step 5: Check final dataset structure and column names
colnames(BexClean)
head(BexClean)  # Preview the first few rows
...
```
  ...
  * Dyad,IEloDiff, IZDSi, AgeDiff, Date, DyadDay, DyadTrial, Resp, Tol, Agg, NoApp, EloSex
  
  *MAge, FAge,MElo, FElo, 
  
  
  * IDsi mus be calculated using the same amount of time before 1st trial (ex. 60 days before 1st date of trial/dyad)
  * Elo Diff, must have the same requirement as IDSI for amount of data taken in account
  * Tol, Agg and NoApp will be binomial variables for Tolerance, Aggression and Not approaching
...
# REMARKS
  * IDSI and IEloDiff will be fixed (Same value in all the data set/dyad) because we selected only the initial differences in rank and social bonds
  * If Tolerance is a proportion it is a bound value btw 1 or 0, beta regression ca be used for bound value or nbinomal model
...
#  TASKS
  ...
  * >Pom Elo from a month after experiment maybe since we started experiment as soon as he arrived in Noha
  * >ElO
      - Rank / Similar, MaleHigh, FemaleHigh but mention that diff, hiereachire for male and female elo but because we suppose interaction higer sex and rank
      - Diff between quartile values > take absolute difference and the 3 categories to see if inclunece of higher sex /rank
      

# QUESTIONS
  * Seasonality?
  * Male Tenure (Years 7 3 cat?) / Female Pregnancy (17= if bb for 3 month = yes if not 0)
  * Level of habituation, food availability (proxmiity with seasonality), temperature, group size, amount of males in group, previous interactions....





Paper to read: Factors affecting tolerance persistence after grooming interactions in wild female vervet monkeys, Chlorocebus pygerythrus

therotical background Background: How do individual fogr relationship , how does life history diff impact how these relationships 



Goal /Cooperation

Evolution of tolerance, creation of relationship, fwhat foactors change that, food eovolution of social bonds, goal, group living male female realtionhsip, friendhsips
