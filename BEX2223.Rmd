...
* **Intrusion & Loosing interest**: I will keep Intrusion: **Intrusion > Loosing interest**
...
* **Aggression & Not Approaching**: I will keep both Aggression > Not approaching

* **Male aggress male** and **Female aggress male**: create a variable called **Aggression** and put **Yes** for every occurrences of Female and Male aggression and **No** when there wasn't
...
```{r Implementation of Hierarchy in DyadResponse, echo=FALSE}

# Function to create a hierarchy for DyadResponse and create new variables
create_hierarchy <- function(df) {
  # Create necessary variables
  df <- df %>%
    mutate(SimultaneousTouch = ifelse(grepl("Tolerance", DyadResponse), "Yes", "No"),
           Hesitant = ifelse(grepl("Not approaching", DyadResponse) & grepl("Tolerance", DyadResponse), "Yes", "No"),
           Intrusion = ifelse(grepl("Intrusion", DyadResponse), "Yes", "No"),
           Aggression = ifelse(grepl("Female aggress male|Male aggress female", DyadResponse), "Yes", "No"))

  # Count occurrences before modifications
  count_before <- nrow(df)
  
  # Apply the hierarchy
  df <- df[!(grepl("Tolerance", df$DyadResponse) & grepl("Female aggress male|Male aggress female", df$DyadResponse)), ]
  df <- df[!(grepl("Not approaching", df$DyadResponse) & grepl("Tolerance", df$DyadResponse)), ]
  df <- df[!(grepl("Losing interest", df$DyadResponse) & grepl("Tolerance", df$DyadResponse)), ]
  df <- df[!(grepl("Intrusion", df$DyadResponse) & grepl("Tolerance", df$DyadResponse)), ]
  df <- df[!(grepl("Losing interest", df$DyadResponse) & grepl("Intrusion", df$DyadResponse)), ]
  df <- df[!(grepl("Not approaching", df$DyadResponse) & grepl("Intrusion", df$DyadResponse)), ]
  df <- df[!(grepl("Distracted", df$DyadResponse) & grepl("Not approaching", df$DyadResponse)), ]

  # Count occurrences after modifications
  count_after <- nrow(df)

  # Display the number of rows changed
  cat("Number of rows changed: ", count_before - count_after, "\n")

  return(df)
}

# Apply the hierarchy function to the data
BexClean <- create_hierarchy(BexClean)

# Function to create dichotomous variables for specific behaviors
create_dichotomous_variables <- function(df) {
  df$IDIndividual1[is.na(df$IDIndividual1)] <- "No individual"

  df$MultipleResponses <- ifelse(
    df$IDIndividual1 != "No individual" & grepl("[;,] ?", df$DyadResponse),
    ">1 Response",
    "Single Response"
  )

  cat("Variables created: MultipleResponses\n")
  return(df)
}
