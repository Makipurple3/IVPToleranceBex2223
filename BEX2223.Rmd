...
* I will make an intermediary graph and a summary to see that state of DyadResponse at the moment

```{r DyadResposne Detailed Multiple Response, echo=FALSE}

# Assuming 'DyadResponse' is the name of your variable

# Step 1: Identify Rows with Multiple Entries
rows_with_multiple_entries <- which(sapply(Bex$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ";"))) > 1))

# Step 2: Extract Unique Combinations After Split
all_combinations <- unlist(lapply(rows_with_multiple_entries, function(row) {
  unique_chars <- unique(strsplit(as.character(Bex$DyadResponse[row]), ";")[[1]])
  combn(unique_chars, 2, FUN = function(x) paste(x, collapse = " & "))
}))

# Step 3: Count Occurrences of Unique Combinations
combination_counts <- table(all_combinations)

# Display Unique Combinations and Counts
cat("Unique Combinations and Counts:\n")
for (entry in names(combination_counts)) {
  cat(sprintf("%-25s %d\n", entry, combination_counts[[entry]]))
}


# Step 4: Summary of the Data
dyad_response_summary <- table(unlist(strsplit(as.character(Bex$DyadResponse), ";")))

# Display a more organized summary
cat("\nSingleResponse Summary of Dyad Response:\n")
for (entry in names(dyad_response_summary)) {
  cat(sprintf("%-25s %d\n", entry, dyad_response_summary[[entry]]))
}
