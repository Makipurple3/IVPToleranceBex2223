...
# Step 1: Ensure dates are treated as Date objects
BexClean <- BexClean %>%
  mutate(Date = as.Date(Date, format="%Y-%m-%d"))
...
# Step 3: Calculate ExperimentDay: Assign a unique day number to each unique date across the dataset
# Calculate ExperimentDay: Starts at 1 and increments by 1 each time the date changes
BexClean <- BexClean %>%
  arrange(Date) %>%  # Ensure the data is ordered by Date
  mutate(ExperimentDay = cumsum(Date != lag(Date, default = first(Date))))

# Ensure the first ExperimentDay starts from 1 instead of 0
BexClean$ExperimentDay <- BexClean$ExperimentDay + 1



# Step 4: Create the Trial variable: Each trial has a unique identifier within each dyad, starting from 1
# We can now group by Dyad to correctly calculate Trial
>>>>>>> Stashed changes
BexClean <- BexClean %>%
  group_by(Dyad) %>%
...
=======
# Step 5: Create the DyadDay variable: Counts the number of unique experiment days for each dyad separately
>>>>>>> Stashed changes
...
=======
# Step 6: Ensure that the Dyad column is still present and inspect the structure of the data
if (!"Dyad" %in% colnames(BexClean)) {
  stop("Error: 'Dyad' column is missing after processing.")
}

# Check the structure and correctness of the final dataset
str(BexClean)


head((BexClean$ExperimentDay))
unique(BexClean$ExperimentDay)
head((BexClean$DaysSinceStart))
...
# Combined Summary for both Trials and Days
>>>>>>> Stashed changes
...
=======
# Step 2: Print the tables with better presentation
# Calculate the total number of unique experiment days
total_experiment_days <- n_distinct(BexClean$Date)

# Print the total number of experiment days
cat("Total Number of Unique Experiment Days:", total_experiment_days, "\n")
...
# 4.Visualizing the data and tests (to sort and clean)
...
BexClean <- BexClean %>%
  select(DaysSinceStart, ExperimentDay, Date, Month, DyadDay, Trial, Time, Hour, Period, Male, Female, Dyad, 
...
<<<<<<< Updated upstream
# 6. Preparation of life history

* I will prepare the life-history file to have information about dyads and audience and import it

```{r Life History, echo =FALSE}
