...
# Function to find, count, and remove specific combinations in the dataframe
...
update_rows <- function(df) {
  ...
  # Delete rows with 'Other' in DyadResponse and 'No Response' in OtherResponse
  criteria_delete <- (df$OtherResponse == "No Response")
  ...
  df_updated <- df %>%
    mutate(
      DyadResponse = case_when(
        ...
        criteria_2101 ~ "Delete",
        criteria_2729 ~ "Delete",
        TRUE ~ DyadResponse
      ),
    ...
    filter(!(criteria_delete | DyadResponse == "Delete"))
  ...
  return(list(df_updated, rows_changed))
}
...
# Function to print rows where 'Other' is present in DyadResponse and additional information
print_other_occurrences <- function(df) {
  # Filter rows where DyadResponse contains 'Other'
  other_rows <- df %>%
    filter(grepl("Other", DyadResponse)) %>%
    mutate(RowNumber = row_number())
  
  # Print the filtered rows using kable
  if (nrow(other_rows) > 0) {
    cat("Remaining rows with 'Other' in DyadResponse:\n")
    print(kable(other_rows %>% select(DyadResponse, RowNumber, OtherResponse), 
                col.names = c("DyadResponse", "Row Number", "OtherResponse")))
  } else {
    cat("No rows found with 'Other' in DyadResponse.\n")
  }
}

# Print rows with 'Other' in DyadResponse
print_other_occurrences(BexClean_updated)




# Function to remove 'Other' from DyadResponse and replace different values in OtherResponse
clean_other_responses <- function(df) {
  # Remove 'Other' from DyadResponse and count changes
  df <- df %>%
    mutate(DyadResponse = gsub("Other", "", DyadResponse))
  changes_phase_1 <- sum(grepl("Other", BexClean$DyadResponse))
  
  # Replace all occurrences different from 'No Response' in OtherResponse and count changes
  changes_phase_2 <- sum(df$OtherResponse != "No Response")
  df <- df %>%
    mutate(OtherResponse = ifelse(OtherResponse != "No Response", "No Response", OtherResponse))
  
  # Return updated dataframe and counts of changes
  return(list(df, changes_phase_1, changes_phase_2))
}

# Apply the function to clean the dataframe
result <- clean_other_responses(BexClean_updated)

# Extract the updated dataframe and counts of changes
BexClean_updated <- result[[1]]
changes_phase_1 <- result[[2]]
changes_phase_2 <- result[[3]]

# Display the number of changes made
cat("Number of occurrences of 'Other' removed from DyadResponse: ", changes_phase_1, "\n")
cat("Number of occurrences replaced with 'No Response' in OtherResponse: ", changes_phase_2, "\n")
...
* Before that I will create some new variables from the output of DyadResponse putting 1 when a behavior occurred and 0 when it did not, and this for the following behaviours: **Not approaching ,Intrusion, Losing interest**
  - **Tolerance**: the variable called **SimulteanousTouch** records all the occurences where we recorded tolerance but actually recorded monkeys touching the boxes at the same time since sometimes other behaviours occured. Since Tis column has already been created I dont have to do anything more for Tolerance
  - **Not approaching** & **Loosing interest** will be used to creat two columns: **Not approaching ID** that will tak the ID of each individual that did not approach and **Losing Interest ID** taking the ID of eahc individual that lost interest. This will allow me to replace these occurrences when implementing the hierarchy hierarchy without loosing any information.


#OTHER UPDATES FROM HERE

```{r DyadResponse Dichotomic Variables Before Hierarchy Not Approaching and Losig interest, echo=FALSE}
...
# Assuming BexClean is your actual dataset, load it here
# For example, if it's a CSV file, you would use:
# BexClean <- read.csv("path_to_your_dataset.csv")

# Define behaviors of interest
behaviors <- c("Not approaching", "Losing interest", "Distracted")

# Count initial occurrences of each behavior
initial_counts <- sapply(behaviors, function(behavior) {
  sum(grepl(behavior, BexClean$DyadResponse, ignore.case = TRUE))
})
cat("Initial counts:\n")
print(initial_counts)

# Create columns for each behavior
for (behavior in behaviors) {
  behavior_col <- paste0("Behavior_", gsub(" ", "_", behavior))
  BexClean[[behavior_col]] <- NA  # Initialize columns with NA
}

# Function to assign names based on behavior position
assign_names_to_behaviors <- function(df, behaviors) {
  for (i in seq_len(nrow(df))) {
    dyad_responses <- unlist(strsplit(as.character(df$DyadResponse[i]), "; "))
    id_individuals <- unlist(strsplit(as.character(df$IDIndividual1[i]), "; "))
    
    # Ensure equal lengths by padding shorter list
    if (length(id_individuals) > length(dyad_responses)) {
      dyad_responses <- rep(dyad_responses, length.out = length(id_individuals))
    } else if (length(dyad_responses) > length(id_individuals)) {
      id_individuals <- rep(id_individuals, length.out = length(dyad_responses))
    }
    
    # Initialize a list to store assignments
    assignments <- list()
    
    for (j in seq_along(dyad_responses)) {
      behavior <- dyad_responses[j]
      individual <- id_individuals[j]
      
      for (b in behaviors) {
        if (grepl(b, behavior, ignore.case = TRUE)) {
          behavior_col <- paste0("Behavior_", gsub(" ", "_", b))
          assignments[[behavior_col]] <- c(assignments[[behavior_col]], individual)
        }
      }
    }
    
    # Assign individuals to behavior columns or mark as "Other Behavior"
    for (b in behaviors) {
      behavior_col <- paste0("Behavior_", gsub(" ", "_", b))
      if (is.null(assignments[[behavior_col]])) {
        df[i, behavior_col] <- "Other Behavior"
      } else {
        df[i, behavior_col] <- paste(unique(assignments[[behavior_col]]), collapse = "; ")
      }
    }
  }
  return(df)
...
BexClean <- assign_names_to_behaviors(BexClean, behaviors)

# Fill empty behavior columns with "Other Behavior"
BexClean <- BexClean %>%
  mutate(across(starts_with("Behavior_"), ~ ifelse(is.na(.), "Other Behavior", .)))

# Select relevant rows where any behavior of interest is present
relevant_rows <- BexClean %>%
  filter(Behavior_Not_approaching != "Other Behavior" | 
         Behavior_Losing_interest != "Other Behavior" |
         Behavior_Distracted != "Other Behavior") %>%
  select(DyadResponse, IDIndividual1, IntruderID, 
         Behavior_Not_approaching, Behavior_Losing_interest, Behavior_Distracted)

# Rename columns for clarity
relevant_rows <- relevant_rows %>%
  rename(`Not approaching ID` = Behavior_Not_approaching,
         `Losing Interest ID` = Behavior_Losing_interest,
         `Distracted ID` = Behavior_Distracted)

# Count the number of rows in each behavior ID column
not_approaching_count <- sum(relevant_rows$`Not approaching ID` != "Other Behavior")
losing_interest_count <- sum(relevant_rows$`Losing Interest ID` != "Other Behavior")
distracted_count <- sum(relevant_rows$`Distracted ID` != "Other Behavior")

# Print final counts
cat("Count of rows in 'Not approaching ID':", not_approaching_count, "\n")
cat("Count of rows in 'Losing Interest ID':", losing_interest_count, "\n")
cat("Count of rows in 'Distracted ID':", distracted_count, "\n")

# Display the head of relevant rows in a knitr table
knitr::kable(head(relevant_rows), caption = "Relevant Rows with Behaviors")

# Improved Error Checking: Identify rows with potential errors
error_rows <- BexClean %>%
  mutate(row_number = row_number()) %>%
  filter((Behavior_Not_approaching == "Other Behavior" & grepl("Not approaching", DyadResponse, ignore.case = TRUE)) |
         (Behavior_Losing_interest == "Other Behavior" & grepl("Losing interest", DyadResponse, ignore.case = TRUE)) |
         (Behavior_Distracted == "Other Behavior" & grepl("Distracted", DyadResponse, ignore.case = TRUE)) |
         (grepl(";", `Behavior_Not_approaching`) & grepl(";", `Behavior_Losing_interest`) & grepl(";", `Behavior_Distracted`)))

if (nrow(error_rows) > 0) {
  cat("Rows with potential errors (line numbers included):\n")
  print(knitr::kable(error_rows %>% select(row_number, everything()), caption = "Rows with Potential Errors"))
} else {
  cat("No rows with potential errors found.\n")
}

# Display the entire dataset in the View window
...
```{r DyadResponse Hierarchy Intrusion & Loosing interest, echo=FALSE}

# Function to count and ensure both 'Not approaching' and 'Losing interest' are kept in DyadResponse
count_and_keep_not_approaching_losing_interest <- function(df) {
  # Count the number of cases with Not approaching & Losing interest
  count_not_approaching_losing_interest <- sum(grepl("Not approaching", df$DyadResponse) & grepl("Losing interest", df$DyadResponse))
  
  # Return the count
  return(count_not_approaching_losing_interest)
}

# Apply the function to the dataframe
count_not_approaching_losing_interest <- count_and_keep_not_approaching_losing_interest(BexClean)

# Display the result
cat("Number of cases with Not approaching & Losing interest: ", count_not_approaching_losing_interest, "\n")
...
# Function to count the number of cases with Intrusion & Not approaching
count_intrusion_not_approaching <- function(df) {
  count_intrusion_not_approaching <- sum(grepl("Intrusion", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  return(count_intrusion_not_approaching)
}

# Apply the function to the dataframe
count_intrusion_not_approaching <- count_intrusion_not_approaching(BexClean)

# Display the result
cat("Number of cases with Intrusion & Not approaching: ", count_intrusion_not_approaching, "\n")

# Function to remove 'Not Approaching' when both 'Intrusion' and 'Not Approaching' are present in DyadResponse
remove_not_approaching_with_intrusion <- function(df) {
  # Count the number of cases with Intrusion & Not approaching
  count_intrusion_not_approaching <- sum(grepl("Intrusion", df$DyadResponse) & grepl("Not approaching", df$DyadResponse))
  
  # Remove 'Not Approaching' when both are present
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Intrusion", DyadResponse) & grepl("Not approaching", DyadResponse), 
  ...
  # Return updated dataframe and counts
  return(list(df, count_intrusion_not_approaching, remaining_intrusion_not_approaching))
}

# Apply the function to the dataframe
result_remove_intrusion <- remove_not_approaching_with_intrusion(BexClean)

# Extract the updated dataframe and counts
BexClean_updated <- result_remove_intrusion[[1]]
changes_made <- result_remove_intrusion[[2]]
remaining_intrusion_not_approaching <- result_remove_intrusion[[3]]

# Display the count of changes made and remaining occurrences
cat("Number of changes made to remove 'Not Approaching' with 'Intrusion': ", changes_made, "\n")
cat("Number of remaining cases with both Intrusion and Not approaching: ", remaining_intrusion_not_approaching, "\n")
...
# Function to count the number of cases with Not approaching & Distracted
count_not_approaching_distracted <- function(df) {
  count_not_approaching_distracted <- sum(grepl("Not approaching", df$DyadResponse) & grepl("Distracted", df$DyadResponse))
  return(count_not_approaching_distracted)
}

# Apply the function to the dataframe
count_not_approaching_distracted <- count_not_approaching_distracted(BexClean)

# Display the result
cat("Number of cases with Not approaching & Distracted: ", count_not_approaching_distracted, "\n")

# Function to remove 'Distracted' when both 'Not approaching' and 'Distracted' are present in DyadResponse
remove_distracted_with_not_approaching <- function(df) {
  # Count the number of cases with Not approaching & Distracted
  count_not_approaching_distracted <- sum(grepl("Not approaching", df$DyadResponse) & grepl("Distracted", df$DyadResponse))
  
  # Remove 'Distracted' when both are present
  df <- df %>%
    mutate(DyadResponse = ifelse(grepl("Not approaching", DyadResponse) & grepl("Distracted", DyadResponse), 
                                 gsub("Distracted", "", DyadResponse), 
                                 DyadResponse))
  
  # Trim any extra whitespace that might have been introduced
  df$DyadResponse <- trimws(df$DyadResponse)
  
  # Count the remaining lines with both Not Approaching and Distracted
  remaining_not_approaching_distracted <- sum(grepl("Not approaching", df$DyadResponse) & grepl("Distracted", df$DyadResponse))
  
  # Return updated dataframe and counts
  return(list(df, count_not_approaching_distracted, remaining_not_approaching_distracted))
}

# Apply the function to the dataframe
result_remove_distracted <- remove_distracted_with_not_approaching(BexClean)

# Extract the updated dataframe and count of changes
BexClean_updated <- result_remove_distracted[[1]]
changes_made <- result_remove_distracted[[2]]
remaining_not_approaching_distracted <- result_remove_distracted[[3]]

# Display the count of changes made and remaining occurrences
cat("Number of changes made to remove 'Distracted' with 'Not approaching': ", changes_made, "\n")
cat("Number of remaining cases with both Not approaching and Distracted: ", remaining_not_approaching_distracted, "\n")
...
# Function to handle Tolerance & Not Approaching cases
