...
```{r aggression temporary graphs, echo=FALSE}
...
overall_aggression_plot <- ggplot(data = overall_aggression, aes(x = DyadResponse, y = Count, fill = DyadResponse)) +
  ...
  theme_minimal(base_size = 15) +
  labs(x = "Aggression Type", y = "Count", title = "Overall Male vs Female Aggression") +
  scale_fill_manual(values = c("Male aggress female" = "blue", "Female aggress male" = "red")) +
  theme(legend.position = "none") +
  geom_hline(aes(yintercept = mean_agg), linetype = "dashed", color = "gray") +  # Add mean line
  annotate("text", x = 1.5, y = mean_agg + 3, label = paste("Mean =", round(mean_agg, 2)), color = "gray", size = 5)
...
# Horizontal bar plot to improve readability
dyad_aggression_plot <- ggplot(data = dyad_aggression, aes(x = Proportion, y = reorder(Dyad, Proportion), fill = DyadResponse)) +
  geom_bar(stat = "identity", position = "fill", width = 0.7) +
  theme_minimal(base_size = 15) +
  labs(x = "Proportion", y = "Dyad", title = "Proportion of Male vs Female Aggression by Dyad") +
...
# Print the dyad aggression proportion plot
print(dyad_aggression_plot)
