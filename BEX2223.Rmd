...
# Function to update specific rows in the dataframe for rows 286 and 300
update_specific_rows_286_300 <- function(df) {
  # Define the criteria to find specific rows
  criteria_286 <- (df$MaleID == "Kom" & df$FemaleID == "Oort" & tolower(df$DyadResponse) == "other" & df$OtherResponse == "Ooet scream while at the box and Kom get the corm of both")
  criteria_300 <- (df$MaleID == "Kom" & df$FemaleID == "Oort" & tolower(df$DyadResponse) == "other" & df$OtherResponse == "Both at boxes; Kom touching the box of Oort")

  # Update the specific rows
  df_updated <- df %>%
    mutate(
      DyadResponse = ifelse(criteria_286, "Tolerance", 
                     ifelse(criteria_300, "Tolerance", DyadResponse)),
      SpecialBehaviour = ifelse(criteria_286, "Kom;Ap;Sf;Oort,Oort;Rt;Sc", 
                        SpecialBehaviour),
      GotCorn = ifelse(criteria_286, "No;Oort", GotCorn)
    )

  # Count the number of rows changed
  rows_changed_286 <- sum(criteria_286)
  rows_changed_300 <- sum(criteria_300)
  total_rows_changed <- rows_changed_286 + rows_changed_300

  # Count the remaining occurrences of 'Other' in DyadResponse
  remaining_other_count <- sum(grepl("Other", df_updated$DyadResponse))
  
  # Return the updated dataframe, number of rows changed, and remaining 'Other' occurrences
  return(list(df_updated, total_rows_changed, remaining_other_count))
}

# Process the remaining rows after the initial removal
result <- update_specific_rows_286_300(BexClean_updated)

# Extract the updated dataframe, rows changed, and remaining 'Other' occurrences
BexClean_final <- result[[1]]
rows_changed <- result[[2]]
remaining_other_count <- result[[3]]

# Display the number of rows changed
cat("Number of rows that have been changed: ", rows_changed, "\n")

# Display the number of occurrences of 'Other' in DyadResponse left
cat("Number of occurrences of 'Other' in DyadResponse left: ", remaining_other_count, "\n")

# You can now use BexClean_final for further analysis
