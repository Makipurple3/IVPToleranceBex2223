...
Bex$MaleID[wrong_rows[921]] <- "Sho"

# Correct "Xin" with "Kom" in rows 2536 to 2539
Bex$MaleID[wrong_rows[2692:2695]] <- "Kom"
...
Bex <- Bex %>% mutate(RowID = row_number())
...
```
* I added a new column RowID that assigns a unique identifier to each row.
I replaced the arrange(Date, Dyad) and arrange(Dyad, Date) with arrange(RowID) to use the unique identifier for ordering.
I removed the RowID column after creating the trial and session variables.
These changes ensure that the code produces consistent results regardless of the order of rows in the dataset.
