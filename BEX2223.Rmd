...
* I will use data files from IVP that contain longterm data as on theirage, agonistic and affiliative exhancges, and other data.


* Female DOB: Direct from the dataset except for Ouli, who needs a manual adjustment.
...
# Step 4: Calculate the age based on FirstRecorded for all individuals
LHdata_females <- LHdata_females %>%
  mutate(
    Age = as.numeric(difftime(Sys.Date(), FirstRecorded, units = "weeks")) / 52.1775  # Age in years
  )

# Step 5: Print the updated dataframe to check if Age is correct
print(LHdata_females %>% select(Code, FirstRecorded, Age))

# Step 6: Create a simple plot of the Ages
library(ggplot2)

ggplot(LHdata_females, aes(x = Code, y = Age)) +
  geom_bar(stat = "identity", fill = "lightblue") +
  theme_minimal() +
  labs(title = "Age of Female Vervets", x = "Individual", y = "Age (Years)") +
