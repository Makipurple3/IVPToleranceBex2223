...
cat("Occurrences of Female IDs:\n")
print(table(BexClean$FemaleID))
...
# Count the occurrences of each name before the transformation
cat("Occurrences of Male IDs:\n")
print(table(BexClean$MaleID))
...
# Cross-tabulate Male and Female IDs
corrected_table <- table(BexClean$Male, BexClean$Female)
kable(corrected_table)
...
wrong_rows <- which(BexClean$Dyad %in% wrong_dyads)
...
# UPDATES FROM HERE
  ...
  * I will change the occurences of **Buk Ginq** to **Sho Ginq** for **row 613 to 617** and **row 931**. I know these triasl are with SHo Ginq because the comments metionned Sho in them while Male(ID) gave Buk
...
if (!all(Bex$MaleID[613:617] == "Sho", na.rm = TRUE)) {
  Bex$MaleID[613:617] <- "Sho"
}

# Check if corrections have already been made for "Sho" at another specific row
if (Bex$MaleID[931] != "Sho") {
  Bex$MaleID[931] <- "Sho"
}

# Check if corrections have already been made for "Kom"
if (!all(Bex$MaleID[2170:2713] == "Kom", na.rm = TRUE)) {
  Bex$MaleID[2170:2713] <- "Kom"
}
