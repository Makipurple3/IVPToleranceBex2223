...
if (!all(BexClean$MaleID[613:617] == "Sho", na.rm = TRUE)) {
  BexClean$MaleID[613:617] <- "Sho"
}

# Check if corrections have already been made for "Sho" at another specific row
if (BexClean$MaleID[931] != "Sho") {
  BexClean$MaleID[931] <- "Sho"
}

# Check if corrections have already been made for "Kom"
if (!all(BexClean$MaleID[2170:2713] == "Kom", na.rm = TRUE)) {
  BexClean$MaleID[2170:2713] <- "Kom"
}

# Delete duplicate rows if any
BexClean <- BexClean[!duplicated(BexClean), ]

# Update Dyad based on the corrected MaleID
BexClean <- dplyr::mutate(BexClean, Dyad = paste(
...
unique_male <- unique(BexClean$MaleID)
cat("Unique Male after correction", unique_male, "\n")

unique_dyad <- unique(BexClean$Dyad)
cat("Unique Dyad after correction", unique_dyad, "\n")

knitr::kable(table(BexClean$MaleID))
knitr::kable(table(BexClean$Dyad))
