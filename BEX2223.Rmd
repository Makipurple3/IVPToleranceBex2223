...
# Calculate frequency of each distance
distance_freq <- BexClean %>%
  count(DyadDistance) %>%
  arrange(DyadDistance)

# Calculate the median distance
median_distance <- median(BexClean$DyadDistance, na.rm = TRUE)

# Create the horizontal bar plot with purple bars
plot_distance_freq <- ggplot(distance_freq, aes(x = n, y = factor(DyadDistance))) +
  ...
  geom_vline(xintercept = median_distance, linetype = "dashed", color = "red", size = 1) +
  annotate("text", x = median_distance + 0.2, y = max(as.numeric(factor(distance_freq$DyadDistance))) + 0.2, label = paste("Median:", median_distance), color = "red", size = 6, hjust = 0, vjust = -0.5) +
...
plot_proximity_freq <- ggplot(proximity_freq, aes(x = n, y = factor(Proximity), fill = Proximity)) +
  geom_bar(stat = "identity", width = 0.4) +
  geom_text(aes(label = n), hjust = -0.2, size = 4, color = "black") +
  labs(title = "Frequency of Proximity Categories",
       y = "Proximity", x = "Frequency (Amount Trials)") +
  theme_minimal() +
  theme(legend.position = "none", text = element_text(size = 14), axis.text.y = element_text(angle = 0, hjust = 1))
