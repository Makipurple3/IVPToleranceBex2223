...
* Unique Combinations and Counts:
 ...
 - Not approaching &  Losing interest 1         - Not approaching > Losing interest: Remove Losing interest if there is Not approaching
...
Bex1 <- Bex[!(grepl("Distracted", Bex$DyadResponse) & grepl("Intrusion", Bex$DyadResponse)), ]

# Count occurrences after modification
after_summary1 <- nrow(Bex1)

# Display the change in occurrences
cat("Change in Occurrences for 1st Chunk (Intrusion > Distracted): ", after_summary1 - before_summary, "\n")

```

2.Female aggress male > Intrusion: Remove intrusion if there is Female aggress male in the same cell(x2)
```{r Female agress male > Intrusion, echoFALSE}
...
# Remove rows with 'Losing interest' if 'Intrusion'and Loosing interest
Bex <- Bex[!(grepl("Losing interest", Bex$DyadResponse) & grepl("Intrusion", Bex$DyadResponse)), ]

```
4. Intrusion > Not approaching: Remove not approaching if there is intrusion in the same cell(1x)
```{r Intrusion > Not approaching, echo=FALSE}
...
{r DyadResposneMore1 Disaply, echo=FALSE}

# Step 2: Display Unique Combinations Before Split
cat("Unique Combinations Before Split:\n")
for (i in seq_along(rows_with_multiple_entries)) {
  row <- rows_with_multiple_entries[i]
  unique_chars <- unique(strsplit(as.character(Bex$DyadResponse[row]), ";")[[1]])
  unique_combinations <- combn(unique_chars, 2, FUN = function(x) paste(x, collapse = " & "))
  cat("Row number: ", row, "\n")
  cat("Combinations: ", toString(unique_combinations), "\n\n")
}
