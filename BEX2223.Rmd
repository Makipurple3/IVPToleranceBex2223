...
# Step 1: Ensure combined_data is ready with male and female data
combined_data <- bind_rows(
  LHdata_females_table %>% mutate(Gender = "Female"),  # Add Gender column for females
  LHdata_males_table %>% mutate(Gender = "Male")       # Add Gender column for males
)

# Step 2: Join male and female age data for each dyad
# Use "Male" and "Female" as the column names
dyad_data <- BexClean %>%
  left_join(combined_data, by = c("Male" = "Code")) %>% 
  rename(MaleAge = Age) %>%
  left_join(combined_data, by = c("Female" = "Code")) %>%
  rename(FemaleAge = Age) %>%
  mutate(AgeDifference = abs(MaleAge - FemaleAge)) %>%  # Calculate absolute age difference
  arrange(AgeDifference)  # Sort by absolute age difference

# Step 3: Plot Absolute Age Difference
ggplot(dyad_data, aes(x = reorder(paste(Male, Female, sep = " - "), AgeDifference), y = AgeDifference)) +
  geom_bar(stat = "identity", fill = "lightblue", width = 0.7) +
  geom_text(aes(label = round(AgeDifference, 2)), vjust = -0.5, size = 4) +
  theme_minimal() +
...
dyad_data <- dyad_data %>%
  mutate(AvgAge = (MaleAge + FemaleAge) / 2,  # Calculate average age in the dyad
         RelativeAgeDifference = (abs(MaleAge - FemaleAge) / AvgAge) * 100)  # Calculate relative age difference

# Check for any NA or invalid AvgAge values
summary(dyad_data$AvgAge)
summary(dyad_data$RelativeAgeDifference)

# Now plot
ggplot(dyad_data, aes(x = reorder(paste(Male, Female, sep = " - "), RelativeAgeDifference), y = RelativeAgeDifference)) +
  geom_bar(stat = "identity", fill = "lightgreen", width = 0.7) +
  geom_text(aes(label = paste(round(RelativeAgeDifference, 2), "%")), vjust = -0.5, size = 4) +
  theme_minimal() +
...
ggplot(dyad_data, aes(x = reorder(paste(Male, Female, sep = " - "), ZScoreAgeDiff), y = ZScoreAgeDiff)) +
...
* For further anaylsis we may consider if the male or female was older 
