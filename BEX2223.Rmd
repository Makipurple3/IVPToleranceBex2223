...
#### 2.1 Merging Male placement corn and MaleCorn
...
```{r Check if Remaing NA, include=FALSE}
...
### Creating New Variables and modifying the existing ones
...
```
1. Create a variavble called Male that in each row will take the name of the **3 letter code** that is either in MaleID or Female ID and a variable called Female that in each row will take the name of the **4 letter code** that is either in MaleID or FemaleID
...
# Replace "Buk" with "Sho" in rows 1 to 5
Bex$MaleID[c(1, 2, 3, 4, 5)] <- "Sho"

# Replace "Xin" with "Kom" in rows 2536 to 2539
Bex$MaleID[c(2536, 2537, 2538, 2539)] <- "Kom"
# Delete row 6
Bex <- Bex[-6, ]


# Check and summarize unique MaleIDs after correction
...
# Check and summarize unique Dyads after correction
unique_dyad <- unique(Bex$Dyad)
...
```
  ...
  * The rows 1 to 6 are not all in the same row of trials. While I found out using comments that Sho was in the first 5 trials, I will change the occurences of **Buk Ginq** to **Sho Ginq** for row 1,2,3,4,5, and delete row 6
  
  * Since Ouli is in the audience it is unlikely that we had trials with the dyad **Xin Ouli** in the rows 2536,2537,2538,2539. Also I think they are little chances that the names of both individuals were entered wrong. I will replace these occurences where we had **Xin Oort** by **Kom Oort**
  
  * I thus want **Buk** to be replaced in male ID in rows 1,2,3,4,5 with **Sho** and **Xin** to be replaced by **Kom** in rows 2536,2537,2538,2539 in Male ID before updating Dyad

  3. Create the variable called **Trial** where the data will be **sorted by date and dyad** in order to see how many trials have been done with each individual: **One row (per dyad) = one trial** and the    variable called **Session** where the data will be **sorted by date and dyad** in order to see how many sessions have been done with each individual: **One day (per dyad) = one session**
  Now, let's proceed with creating the Dyad variable, Trial, and Session:

```{r Dyad and Session, echo=FALSE}
...
summary_session <- Bex %>%
  group_by(Dyad) %>%
  summarise(Num_Sessions = max(Session))

cat("Trial Summary:\n")
print(summary_trial)

cat("\nSession Summary:\n")
print(summary_session)

kable(table(Bex$Dyad))

```
  4. Correct the wrong dyads that are in Male and Female to correct Dyad
  
  * Looking at the summary of the Dyad column, I saw that some 
  
  5. Make a summary   of trial and session so I can see see how many trials and sessions have been done with the individuals
