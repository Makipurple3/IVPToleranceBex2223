...
# Step 1: Reorder and select necessary columns
if (!all(c("Male", "Female", "Dyad", "Date", "TrialDay", "DyadDay", "Trial", 
           "Period", "Month", "DyadResponse") %in% colnames(BexClean))) {
  stop("One or more expected columns are missing in 'BexClean'. Please check column names.")
}

# Select relevant columns
BexClean <- BexClean %>%
  select(Male, Female, Dyad, Date, TrialDay, DyadDay, Trial, Period, Month, DyadResponse)

# Step 2: Count unique DyadResponse values and their frequencies
response_counts <- table(BexClean$DyadResponse)

# Print unique DyadResponse values and their counts
cat("Unique DyadResponse values and their counts:\n")
print(response_counts)

# Count the total number of unique response types
num_unique_responses <- length(unique(BexClean$DyadResponse))
cat("\nTotal number of unique response types:", num_unique_responses, "\n")

# Step 3: Create columns for counts of each response type using pattern matching
# Assign binary indicators for each response type based on partial matches
BexClean <- BexClean %>%
  mutate(
    Tol = ifelse(grepl("Tolerance", DyadResponse, ignore.case = TRUE), 1, 0),
    Agg = ifelse(grepl("male aggress female|female aggress male", DyadResponse, ignore.case = TRUE), 1, 0),
    NotApp = ifelse(grepl("Not approaching", DyadResponse, ignore.case = TRUE), 1, 0),
    Intrud = ifelse(grepl("Intrusion", DyadResponse, ignore.case = TRUE), 1, 0)
  )

# Step 4: Verify the counts using distinct matches
# Ensure all variations are considered
tol <- sum(BexClean$Tol, na.rm = TRUE)
agg <- sum(BexClean$Agg, na.rm = TRUE)
notapp <- sum(BexClean$NotApp, na.rm = TRUE)
intrud <- sum(BexClean$Intrud, na.rm = TRUE)

# Print the counts
cat("Tolerance Count:", tol, "\n")
cat("Aggression Count:", agg, "\n")
cat("Not Approaching Count:", notapp, "\n")
cat("Intrusion Count:", intrud, "\n")
...
write.csv(BexClean, file =  "//Users/maki/Desktop/Master Thesis/BEX 2223 Master Thesis Maung Kyaw/IVPToleranceBex2223/BexStat.csv", row.names = FALSE)

# Print confirmation message
cat("CSV file successfully saved at:",  "//Users/maki/Desktop/Master Thesis/BEX 2223 Master Thesis Maung Kyaw/IVPToleranceBex2223/BexStat.csv", "\n")
...
pTol ~ AmountTrial + AgeDiff + IEloDiff + IDsi + (Day | Dyad)


# Example GLMM in R
library(lme4)
model <- glmer(
  pTol ~ AmountTrial + AgeDiff + IEloDiff + IDsi + (1 | Dyad), 
  data = your_data, 
