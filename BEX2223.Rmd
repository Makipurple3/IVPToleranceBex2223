...
BexClean_backup <- BexClean

# Function to safely update rows based on specific criteria
update_row <- function(df, male_id, female_id, other_response, dyad_response, special_behaviour, got_corn) {
  row_index <- which(df$MaleID == male_id & df$FemaleID == female_id & df$OtherResponse == other_response)
  if (length(row_index) == 1) {
    if (!is.null(dyad_response)) {
      df$DyadResponse[row_index] <- dyad_response
    }
    if (!is.null(special_behaviour)) {
      df$SpecialBehaviour[row_index] <- special_behaviour
    }
    if (!is.null(got_corn)) {
      df$GotCorn[row_index] <- got_corn
    }
  } else if (length(row_index) == 0) {
    cat("Warning: No rows matched for MaleID:", male_id, ", FemaleID:", female_id, ", OtherResponse:", other_response, "\n")
  } else {
    cat("Warning: Multiple rows matched for MaleID:", male_id, ", FemaleID:", female_id, ", OtherResponse:", other_response, "\n")
  }
  return(df)
}

# Apply modifications based on your specifications
BexClean <- update_row(BexClean, "Buk", "Ndaw", "No Response", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Kom", "Oort", "Ooet scream while at the box and Kom get the corm of both", "Tolerance", "Kom;Ap;Sf;Oort,Oort;Rt;Sc", "No;Oort")
BexClean <- update_row(BexClean, "Kom", "Oort", "Both at boxes; Kom touching the box of Oort", "Tolerance", NULL, NULL)
BexClean <- update_row(BexClean, "Kom", "Oort", "Kom touching Oort’s box. Oort came after 30 sec to her own box.", "Tolerance", "Oort;Ap;After30", NULL)
BexClean <- update_row(BexClean, "Nge", "Oerw", "No Response", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Pom", "Xian", "No Response", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Pom", "Xian", "No Response", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Sey", "Sirk", "wait for her. moved back and waited for sirk to approach and came back", "Tolerance", "Sirk;ApproachWithPartner", NULL)
BexClean <- update_row(BexClean, "Sho", "Ginq", "No Response", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Sho", "Ginq", "Sho took one of her corn", NULL, "Sho;Sf;Ginq", "No;Ginq")
BexClean <- update_row(BexClean, "Xia", "Piep", "aggression", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Xia", "Piep", "Xia ate corn from both boxes", "Tolerance", "Xia;Sf;Piep", "No;Piep")
BexClean <- update_row(BexClean, "Xia", "Piep", "Xia stolen some pieces from piep box", "Tolerance", "Xia;Sf;Piep", "No;Piep")
BexClean <- update_row(BexClean, "Xia", "Piep", "Xia immediately go for piep box and eat her corn then eat his.", "Tolerance", "Xia;Sf;Piep", "No;Piep")
BexClean <- update_row(BexClean, "Xia", "Piep", "piep got 2 pieces out of 3. Xia took one corn", "Tolerance", "Xia;Sf;Piep", "No;Piep")
BexClean <- update_row(BexClean, "Piep", "Xia", "fem ag male", "Female aggress male", NULL, NULL)
BexClean <- update_row(BexClean, "Xia", "Piep", "Neither ID approach", NULL, NULL, NULL)
BexClean <- update_row(BexClean, "Xin", "Ouli", "opened for oerw", NULL, NULL, NULL)

# Remove rows with NULL in DyadResponse and those with 'No Response'
BexClean <- BexClean %>% filter(!is.null(DyadResponse) & DyadResponse != "No Response")

# Print the number of rows changed
changed_rows_count <- nrow(BexClean_backup) - nrow(BexClean)
cat("Number of rows that have been changed: ", changed_rows_count, "\n")

# Display the updated rows with "Other" in DyadResponse
