...
#Identify existing rows in BexClean
colnames(BexClean)

# Load required libraries
library(dplyr)

# Ensure `BexClean` is the cleaned dataset containing necessary variables
# Key columns include MaleDOB, FemaleDOB, MEloQuartile, FEloQuartile, etc.

# 1. Compute MAge, FAge, and AgeDiff


# 2. Compute IEloDiff using Quartile Information


# 3. Compute Binomial Variables (Tol, Agg, NotApp)
BexClean<- BexClean %>%
  mutate(
    Tol = ifelse(DyadResponse == "Tolerance", 1, 0),  # Tolerance as binomial
    Agg = ifelse(DyadResponse == "Aggression", 1, 0),  # Aggression as binomial
    NotApp = ifelse(DyadResponse == "NotApproaching", 1, 0)  # NotApproaching as binomial
  )

# 4. Add Existing and Temporal Variables

# 5. Ensure all required variables exist
