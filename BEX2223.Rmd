...
* Before that I will create some new variables from the output of DyadResponse putting 1 when a behavior occurred and 0 when it did not, and this for the following behaviours: **Tolerance, Not approaching ,Male aggress female, Intrusion, Losing interest, Female aggress male*
...
```{r DyadResponse Dichotomic Variables Before Hierarchy Not Approaching, echo=FALSE}


```


### Part 2: Selecting Relevant Rows, Counting, and Error Identification

This part handles selecting relevant rows, renaming columns, counting final occurrences, and identifying rows with potential errors.


```{r DyadResponse Dichotomic Variables Before Hierarchy Final Steps, echo=FALSE}
...
for (behavior in behaviors) {
  BexClean[[paste0("Behavior_", gsub(" ", "_", behavior))]] <- ""
}

# Function to assign names based on behavior position
assign_names_to_behaviors <- function(df, behaviors) {
  for (i in seq_len(nrow(df))) {
    dyad_responses <- unlist(strsplit(df$DyadResponse[i], "; "))
    id_individuals <- unlist(strsplit(df$IDIndividual1[i], "; "))
    ...
    for (j in seq_along(dyad_responses)) {
      ...
      for (b in behaviors) {
        if (grepl(b, behavior)) {
          ...
          df[i, behavior_col] <- ifelse(df[i, behavior_col] == "", individual, paste(df[i, behavior_col], individual, sep = "; "))
        }
      }
    }
  }
...
BexClean <- BexClean %>%
  mutate(across(starts_with("Behavior_"), ~ ifelse(. == "", "Other Behavior", .)))
...
relevant_rows <- BexClean %>%
  filter(Behavior_Not_approaching != "Other Behavior" | Behavior_Losing_interest != "Other Behavior") %>%
  select(DyadResponse, IDIndividual1, IntruderID, 
         Behavior_Not_approaching, Behavior_Losing_interest)
...
# Identify rows with potential errors
error_rows <- BexClean %>%
  filter((Behavior_Not_approaching == "Other Behavior" & grepl("Not approaching", DyadResponse)) |
         (Behavior_Losing_interest == "Other Behavior" & grepl("Losing interest", DyadResponse)))

if (nrow(error_rows) > 0) {
  cat("Rows with potential errors:\n")
  print(knitr::kable(error_rows, caption = "Rows with Potential Errors"))
} else {
