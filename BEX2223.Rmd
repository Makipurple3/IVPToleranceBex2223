...
# Step 3: Filter female individuals (use the subset of individuals you're working with)
...
LHdata_females <- LHdata_females %>%
  mutate(
    AdjustedDOB = ifelse(Code == "Ouli", as.character(FirstRecorded - years(4)), as.character(DOB)),  # Subtract 4 years for Ouli
    AdjustedDOB = ymd(AdjustedDOB)  # Parse AdjustedDOB as a date using lubridate's ymd() function
  )
...
# Step 2: Print the DOB and FirstRecorded for each male
print(LHdata_males %>% select(Code, DOB, FirstRecorded))
...
# Step 3: Filter male individuals (the subset of individuals you're working with)
males <- c("Sho", "Buk", "Kom", "Nge", "Xia", "Xin", "Sey", "Pom")
LHdata_males <- LHdata %>%
  filter(Code %in% males)

# Step 4: Adjust DOB for males; if DOB is missing, subtract 5 years from FirstRecorded
LHdata_males <- LHdata_males %>%
  mutate(
    AdjustedDOB = ifelse(is.na(DOB), as.character(FirstRecorded - years(5)), as.character(DOB)),  # Subtract 5 years if DOB is missing
    AdjustedDOB = ymd(AdjustedDOB)  # Parse AdjustedDOB as a date using lubridate's ymd() function
  )
...
# Use the Gender column to differentiate between males and females
combined_data <- bind_rows(
  LHdata_females_table %>% mutate(Gender = "Female"),  # Add Gender column for females
  LHdata_males_table %>% mutate(Gender = "Male")       # Add Gender column for males
)
...
ggplot(combined_data, aes(x = reorder(Code, Age), y = Age, fill = Gender)) +
  geom_bar(stat = "identity") +  # Bar plot for ages
  geom_hline(yintercept = mean_age, linetype = "dashed", color = "red", size = 1) +  # Mean age line
  theme_minimal() +
  labs(title = "Age of Male and Female Vervet Monkeys with Mean Age", x = "Individual", y = "Age (Years)") +
  scale_fill_manual(values = c("Male" = "lightblue", "Female" = "purple")) +  # Color coding for Male and Female
  geom_text(aes(label = round(Age, 2)), vjust = -0.5) +  # Add age values on bars
...
# Step 1: Ensure the Dyad column is in the format "Male Female"
bex_dyads <- BexClean %>%
  mutate(Dyad = paste(Male, Female)) %>%  # Create the Dyad column as "Male Female"
  select(Dyad, Male, Female) %>%
  distinct()  # Ensure distinct dyads
...
dyad_with_male_age <- filtered_dyads %>%
  left_join(LHdata_males_table %>% select(Code, Age) %>% rename(MaleAge = Age, Male = Code), by = "Male")

# Join Female age data
dyad_with_age <- dyad_with_male_age %>%
  left_join(LHdata_females_table %>% select(Code, Age) %>% rename(FemaleAge = Age, Female = Code), by = "Female")

# Step 5: Calculate the absolute age difference
dyad_with_age <- dyad_with_age %>%
  mutate(AgeDifference = abs(MaleAge - FemaleAge))

# Step 6: Filter out rows with missing values in MaleAge or FemaleAge to keep only clean data
dyad_data_clean <- dyad_with_age %>%
  filter(!is.na(MaleAge) & !is.na(FemaleAge))

# Step 7: Display the final clean data for verification
dyad_table <- dyad_data_clean %>%
  select(Dyad, Male, Female, MaleAge, FemaleAge, AgeDifference)

# Print the clean table to verify
print(dyad_table)
...
# Assuming dyad_data_clean already contains the AgeDifference column

# Step 1: Create a bar plot of the absolute age differences between the dyads
ggplot(dyad_data_clean, aes(x = reorder(Dyad, AgeDifference), y = AgeDifference)) +
  geom_bar(stat = "identity", fill = "#87CEEB", width = 0.7) +
  geom_text(aes(label = round(AgeDifference, 2)), vjust = -0.5, size = 4, color = "black", fontface = "bold") +
  theme_minimal() +
  labs(title = "Absolute Age Difference Between Dyads", x = "Dyad", y = "Absolute Age Difference (Years)") +
...
# Step 1: Calculate Relative Age Difference
dyad_data_clean <- dyad_data_clean %>%
  mutate(RelativeAgeDifference = (abs(MaleAge - FemaleAge) / ((MaleAge + FemaleAge) / 2)) * 100)

# Step 2: Plot Relative Age Difference
ggplot(dyad_data_clean, aes(x = reorder(paste(Male, Female, sep = " - "), RelativeAgeDifference), y = RelativeAgeDifference)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10)) +  # Adjust font size if needed
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 10))  # Adjusting the y-axis to the appropriate range
...
# Step 1: Calculate Z-Score for Age Difference
dyad_data_clean <- dyad_data_clean %>%
  mutate(MeanAgeDiff = mean(AgeDifference, na.rm = TRUE),  # Calculate mean age difference
         SDAgeDiff = sd(AgeDifference, na.rm = TRUE),      # Calculate standard deviation of age difference
         ZScoreAgeDiff = (AgeDifference - MeanAgeDiff) / SDAgeDiff)  # Calculate Z-score for age difference

# Step 2: Plot Z-Scores for Age Difference
ggplot(dyad_data_clean, aes(x = reorder(paste(Male, Female, sep = " - "), ZScoreAgeDiff), y = ZScoreAgeDiff)) +
...
# Step 1: Create the age direction column (whether male or female is older)
dyad_data_clean <- dyad_data_clean %>%
  mutate(AgeDirection = ifelse(MaleAge > FemaleAge, "Male Older", "Female Older"))

# Step 2: Create a summary table with all the required columns
summary_table <- dyad_data_clean %>%
  select(Male, Female, MaleAge, FemaleAge, AgeDirection, AgeDifference, RelativeAgeDifference, ZScoreAgeDiff)

# Step 1: Create the simplified summary table with only the relevant columns
simplified_summary_table <- summary_table %>%
  select(Male, Female, AgeDifference, AgeDirection)

# Step 2: Display the simplified summary table
simplified_summary_table





# Step 1: Create the bar plot visualization
ggplot(simplified_summary_table, aes(x = reorder(paste(Male, Female, sep = " - "), AgeDifference), y = AgeDifference, fill = AgeDirection)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Male Older" = "#1f77b4", "Female Older" = "#ff7f0e")) +  # Custom colors for AgeDirection
...
# Step 1: Ensure that `ToleranceBinomial` is correctly calculated in BexClean
BexClean <- BexClean %>%
  mutate(ToleranceBinomial = ifelse(DyadResponse == "Tolerance", 1, 0))

# Step 2: Create a summary table with raw values of Tolerance and No Tolerance
...
# Check if the proportions are correctly calculated in the summary table
print(tolerance_summary_table)

# Step 3: Plot raw counts of Tolerance and No Tolerance per Dyad (absolute counts)
raw_tolerance_graph <- ggplot(tolerance_summary_table, aes(x = reorder(Dyad, TotalTrials), y = TotalTrials, fill = Dyad)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  geom_text(aes(label = paste0("Tolerance: ", ToleranceCount, "\nNo Tolerance: ", NoToleranceCount)), 
            vjust = -0.5, size = 3.5) +
  labs(title = "Raw Counts of Tolerance and No Tolerance per Dyad", x = "Dyad", y = "Number of Trials") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("#FF7F0E", "#1F77B4", "#2CA02C", "#D62728", "#9467BD", "#8C564B", "#E377C2", "#7F7F7F"))

# Step 4: Order by ToleranceProportion and plot the proportion of Tolerance vs. No Tolerance per Dyad
# Arrange after proportions are calculated and ensure that column exists
tolerance_proportion_graph <- tolerance_summary_table %>%
  pivot_longer(cols = c(ToleranceProportion, NoToleranceProportion), 
               names_to = "ResponseType", values_to = "Proportion") %>%
  arrange(desc(ToleranceProportion))  # Arrange based on calculated tolerance proportion

# Plotting the proportion graph after arranging
proportion_tolerance_graph <- ggplot(tolerance_proportion_graph, aes(x = reorder(Dyad, ToleranceProportion), y = Proportion, fill = ResponseType)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  geom_text(aes(label = paste0(round(Proportion * 100, 2), "%")), vjust = -0.5, size = 3.5) +
  labs(title = "Proportion of Tolerance vs. No Tolerance per Dyad (Ordered by Tolerance)", 
       x = "Dyad", y = "Proportion") +
  theme_minimal() +
  scale_fill_manual(values = c("turquoise", "coral"), labels = c("Tolerance", "No Tolerance")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(labels = scales::percent)

# Step 5: Print the updated summary table for verification
print(tolerance_summary_table)

# Display the graphs
print(raw_tolerance_graph)
print(proportion_tolerance_graph)
