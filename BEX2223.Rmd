...
```{r UniqueResponse Test1, echo=FALSE}
...
cat("Unique Responses and Counts:\n")
for (entry in names(unique_responses_summary)) {
  cat(sprintf("%-25s %d\n", entry, unique_responses_summary[[entry]]))
}


```

```{r TwoResponse Test2, echo=FALSE}


# Identify Rows with Exactly 2 Entries
rows_with_two_entries <- which(sapply(BexClean$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ";"))) == 2))

# Display Rows with Exactly 2 Entries
cat("Number of rows with exactly 2 entries in DyadResponse: ", length(rows_with_two_entries), "\n")

# Extract and Count Unique Combinations for Exactly 2 Entries
all_combinations_2 <- unlist(lapply(rows_with_two_entries, function(row) {
  unique_chars <- unlist(strsplit(as.character(BexClean$DyadResponse[row]), ";"))
  apply(combn(unique_chars, 2), 2, paste, collapse = " & ")
}))

# Count Occurrences of Unique Combinations for Exactly 2 Entries
combination_counts_2 <- table(all_combinations_2)
cat("Unique Combinations and Counts for Exactly 2 Entries:\n")
for (entry in names(combination_counts_2)) {
  cat(sprintf("%-25s %d\n", entry, combination_counts_2[[entry]]))
}
...
```{r MoreThanTwoResponse, echo=FALSE}

# Identify Rows with More than 2 Entries
rows_with_more_than_two_entries <- which(sapply(BexClean$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ";"))) > 2))

# Display Rows with More than 2 Entries
cat("Number of rows with more than 2 entries in DyadResponse: ", length(rows_with_more_than_two_entries), "\n")

# Extract and Count Unique Combinations for More than 2 Entries
if(length(rows_with_more_than_two_entries) > 0) {
  all_combinations_more_than_2 <- unlist(lapply(rows_with_more_than_two_entries, function(row) {
    unique_chars <- unlist(strsplit(as.character(BexClean$DyadResponse[row]), ";"))
    paste(unique_chars, collapse = " & ")
  }))
  
  # Count Occurrences of Unique Combinations for More than 2 Entries
  combination_counts_more_than_2 <- table(all_combinations_more_than_2)
  cat("Unique Combinations and Counts for More than 2 Entries:\n")
  for (entry in names(combination_counts_more_than_2)) {
    cat(sprintf("%-25s %d\n", entry, combination_counts_more_than_2[[entry]]))
...
```{r DyadResponse unique combinations and frequencies, echo=FALSE}
