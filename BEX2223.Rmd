...
```{r chunk for problematic rows in DyadResponse Other, echo=FALSE}
...
update_rows <- function(df) {
  ...
  # Count occurrences of "Other" in DyadResponse
  count_other_in_dyad <- sum(df_updated$DyadResponse == "Other")
  
  # Return the updated dataframe and number of rows changed
  return(list(df_updated, rows_changed, count_other_in_dyad))
}

# Process the dataframe to update rows based on criteria
result <- update_rows(BexClean)

# Extract the updated dataframe, rows changed, and count of "Other" in DyadResponse
BexClean_updated <- result[[1]]
rows_changed <- result[[2]]
count_other_in_dyad <- result[[3]]

# Display the number of rows changed
cat("Number of rows changed: ", rows_changed, "\n")

# Display the count of occurrences of 'Other' in DyadResponse
cat("Occurrences of 'Other' in DyadResponse: ", count_other_in_dyad, "\n")
...
print_other_occurrences <- function(df) {
  ...
  if (nrow(other_rows) > 0) {
    cat("Rows with 'Other' in DyadResponse:\n")
    print(kable(other_rows, col.names = colnames(df),
                caption = "Rows with 'Other' in DyadResponse"))
    
    # Display the number of remaining rows with 'Other' in DyadResponse
    cat("\nNumber of rows with 'Other' in DyadResponse: ", nrow(other_rows), "\n")
    
    # Display the row numbers of the remaining rows with 'Other' in DyadResponse
    remaining_row_numbers <- which(grepl("Other", df$DyadResponse))
    cat("Row numbers with 'Other' in DyadResponse: ", paste(remaining_row_numbers, collapse = ", "), "\n")
  } else {
...
* If there is **Aggression & Tolerance** I will keep **Agression** as we defined tolerance as the absence of any sign of aggression between individuals of a Dyad. Each Time we wrote tolerance it means that the monkeys touched the boxes at the same time. I will create a variable called **SimulatenousTouch** to record every time the box did the action at the same time. Then I will in every case of aggression and tolerance within a trial keep Aggression: **Aggression > Tolerance**

* If there is **Tolerance & Not approaching** I will keep **Tolerance** as it means that one of the individuals took more than 30 seconds to come at the box and then at the end came. In these cases 
...
* If there is **Tolerance & Loosing interest** I will keep **Tolerance** as it means that the dyad did touch the boxes: **Tolerance > Loosing interest**

* If there is **Tolerance & Intrusion** I will keep **Tolerance**. Before replacing these occurrences I will created a variable called **Intrusion** the value will be Yes if there was Intrusion in DyadResponse (alone or with another response) and No if there wasn't any Intrusion: **Tolerance > Intrusion**

* If there is **Not approaching & Loosing interest** I will keep both as **Not approaching = looking box but not coming to it ** while **Loosing interest = not paying attention to the box and not coming to it**

* If there is **Intrusion & Loosing interest** I will keep Intrusion: **Intrusion > Loosing interest**

* If there is **Intrusion & Not approaching** I will keep Intrusion: **Intrusion > Not approaching**

* If there is **Not approaching & Distracted** I will keep **Not approaching**: **Not approaching > Distracted**

* If there is **Aggression & Not Approaching** I will keep both Aggression > Not approaching

* For all occurrences of **Male aggress male** and **Female aggress male** create a variable called **Aggression** and put **Yes** for every occurrences of Female and Male aggression and **No** when there wasn't
...
# 3.9.4 Implementing changes in DyadResponse and Implementing Hierarchy
# Helper function to remove specific combinations
remove_combinations <- function(df, keep, remove) {
  df <- df[!(grepl(remove, df$DyadResponse) & grepl(keep, df$DyadResponse)), ]
  return(df)
}

# 1. Aggression > Tolerance
BexClean <- remove_combinations(BexClean, "Female aggress male", "Tolerance")
BexClean <- remove_combinations(BexClean, "Male aggress female", "Tolerance")

# 2. Tolerance > Not approaching
BexClean <- remove_combinations(BexClean, "Tolerance", "Not approaching")

# 3. Tolerance > Losing interest
BexClean <- remove_combinations(BexClean, "Tolerance", "Losing interest")

# 4. Tolerance > Intrusion
BexClean <- remove_combinations(BexClean, "Tolerance", "Intrusion")

# 5. Intrusion > Losing interest
BexClean <- remove_combinations(BexClean, "Intrusion", "Losing interest")

# 6. Intrusion > Not approaching
BexClean <- remove_combinations(BexClean, "Intrusion", "Not approaching")

# 7. Not approaching > Looks at partner
BexClean$DyadResponse <- gsub("Looks at partner", "Not approaching", BexClean$DyadResponse)

# 8. Not approaching > Distracted
BexClean <- remove_combinations(BexClean, "Not approaching", "Distracted")

# 9. Other > Look case by case and categorize depending on behavior
# This step requires manual inspection and categorization
