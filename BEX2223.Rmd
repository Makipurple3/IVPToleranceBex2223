...
Since the column remarks contains corrections and additional information, so i will treat it now
...
* Now that I have ... I will reimport the dataset and make a check of the data
```{r BexImport After Manual Cleaning, echo=FALSE}
# Import the Excel file
BexClean<- read_excel("Bex_After_manual_cleaning.xlsx")

#Glimpse of Bex for comparaison
glimpse(Bex)

#GLimpse of Bex_Cleaned
glimpse(BexClean)


```

* I will now check the state fo remarks and of the data before and after to see if no major mistakes may have been done

* First the amount of Remarks and No Remarks, before: Number of 'No Remarks' entries:  2119 
Number of actual remarks entries:  599 

```{r temporary check for remarks treatment,echo=FALSE}


# Count the 'No Remarks' and actual remarks entries
no_remarks_count <- sum(BexClean$Remarks == 'No Remarks', na.rm = TRUE)
remarks_count <- sum(BexClean$Remarks != 'No Remarks' & !is.na(BexClean$Remarks), na.rm = TRUE)

# Print the counts
cat("Number of 'No Remarks' entries: ", no_remarks_count, "\n")
cat("Number of actual remarks entries: ", remarks_count, "\n")

# Count total keyword occurrences in the Remarks column
# Replace 'keyword' with the actual keyword you are looking for
# Example keyword, replace 'keyword' with your specific keyword
keyword_occurrences <- sum(grepl("keyword", BexClean$Remarks, ignore.case = TRUE), na.rm = TRUE)

# Print the keyword occurrences
cat("Total number of keyword occurrences in the Remarks column: ", keyword_occurrences, "\n")
```
...
### Treatment and cleaning of last variables before stats!?

### 4.2 Time - Creation of Period and Hour
...
beforesummary <- table(Bex$FemaleID, Bex$MaleID)
kable(print(beforesummary))
...
New output! : 2673 trials 
Old output  : 2724 trials
...
if (!all(Bex$MaleID[605:609] == "Sho", na.rm = TRUE)) {
  Bex$MaleID[605:609] <- "Sho"
}

# Check if corrections have already been made for "Sho" at another specific row
if (Bex$MaleID[921] != "Sho") {
  Bex$MaleID[921] <- "Sho"
}

# Check if corrections have already been made for "Kom"
if (!all(Bex$MaleID[2692:2695] == "Kom", na.rm = TRUE)) {
...
Bex <- dplyr::mutate(Bex, Dyad = paste(
...
knitr::kable(table(Bex$MaleID))
knitr::kable(table(Bex$Dyad))
...
# Rows with more than 1 character
beforesummaryrowswithmultipleentries <- which(sapply(Bex$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ";"))) > 1))

# Display the rows with multiple entries in DyadResponse
View(Bex[beforesummaryrowswithmultipleentries, ])
...
# Escape underscores in output for LaTeX
escaped_rows <- paste(gsub("_", "\\\\_", beforesummaryrowswithmultipleentries), collapse = ", ")
cat("Rows with multiple entries in DyadResponse: ", escaped_rows, "\n")
...
beforesummaryrowswithmultipleentries_1 <- which(sapply(Bex$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ";"))) > 1))

Display the rows with more than 1 entry in DyadResponse
knitr::kable(Bex[beforesummaryrowswithmultipleentries_1, ])
