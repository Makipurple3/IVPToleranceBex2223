...
* Now that I know that they are 230 rows with multiple entries I will print each combinations to see which one I have to print. 
* I will make an intermediary graph and a summary to see that state of DyadResponse at the moment

```{r DyadResponse Sumamry, echo=FALSE}
# Assuming 'DyadResponse' is the name of your variable
rows_with_multiple_entries <- which(sapply(Bex$DyadResponse, function(x) length(unlist(strsplit(as.character(x), ","))) > 1))

# Display unique combinations of entries for rows with multiple entries in DyadResponse
unique_combinations <- lapply(rows_with_multiple_entries, function(row) unique(unlist(strsplit(as.character(Bex$DyadResponse[row]), ","))))

# Display every combination of entries
for (i in seq_along(rows_with_multiple_entries)) {
  row <- rows_with_multiple_entries[i]
  entries <- unique_combinations[[i]]
  cat("Row number: ", row, "\n")
  cat("Entries: ", toString(entries), "\n\n")
}

# Summary of the data
summary_DyadResponse <- summary(Bex$DyadResponse)
cat("Summary Dyad Response:\n")
print(summary_DyadResponse)

# Graph of DyadResponse and the amount of each output
library(ggplot2)

ggplot(Bex, aes(x = DyadResponse)) +
  geom_bar() +
  labs(title = "Distribution of DyadResponse")
