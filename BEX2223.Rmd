...
# Ensure that Context and SpecialBehaviour columns exist
if (!("Context" %in% names(Bex))) {
    Bex[, Context := "No Context"]
}
if (!("SpecialBehaviour" %in% names(Bex))) {
    Bex[, SpecialBehaviour := "No"]
}


# Display all rows containing 'alarm' to understand the current data state, using a case-insensitive search
print(Bex[grep("alarm", tolower(Remarks)), .(Remarks, Context, SpecialBehaviour)])

# Apply changes for the "Alarm" event if it hasn't been treated yet
Bex[tolower(Remarks) %like% "alarm" & Remarks != "Treated", 
    `:=` (Context = "Alarm Event", 
          SpecialBehaviour = "Back to box; Kom", 
          Remarks = "Treated")]

# Confirm the changes by filtering for the updated "Context"
print(Bex[Context == "Alarm Event", .(Remarks, Context, SpecialBehaviour)])

# Additionally, verify changes by directly referencing the row if the index is known
print(Bex[.I == 1816, .(Remarks, Context, SpecialBehaviour)])
...
# Display the row with keyword "foraging" before changes to ensure it exists
print(Bex[Remarks %like% "foraging", .(Remarks, Context)])

# Update safely for the "Foraging" event if it hasn't been treated yet
Bex[Remarks %like% "foraging" & Remarks != "Treated", 
    `:=` (Context = "Foraging", Remarks = "Treated")]

# Confirm the changes by filtering with the updated content
print(Bex[Context == "Foraging", .(Remarks, Context, SpecialBehaviour)])

# #Check the of the row from row number and not keyword 
print(Bex[1892, .(Remarks, Context, SpecialBehaviour)])
...
# Initialize 'Got Corn' if not present and apply changes
if (!"Got Corn" %in% names(Bex)) {
    Bex[, `Got Corn` := "Yes"]
}

# Display the rows before changes
print(Bex[Remarks %in% c("Sirk was scared by Oerw", "oerw was scared to take the corn and nge ate her corn"), .(Remarks, Context, SpecialBehaviour, `Got Corn`)])


# Batch update for "Scare" event safely, avoiding direct row references
Bex[Remarks %in% c("Sirk was scared by Oerw", "oerw was scared to take the corn and nge ate her corn") & Remarks != "Treated", 
    `:=` (SpecialBehaviour = fifelse(Remarks == "Sirk was scared by Oerw", "Scared; Sirky by Oerw", "Scared; Oerw by Nge"),
          Context = fifelse(Remarks == "Sirk was scared by Oerw", "Intrusion", Context),
          `Got Corn` = fifelse(Remarks == "oerw was scared to take the corn and nge ate her corn", "No; Oerw", `Got Corn`),
          Remarks = "Treated")]

# Verify the changes for the involved rows after update
print(Bex[Remarks == "Treated" & (Context == "Intrusion" | `Got Corn` == "No; Oerw"), .(Remarks, Context, SpecialBehaviour, `Got Corn`)])

# Check the row numbers 52 and 2249 after changes
print(Bex[.I %in% c(52, 2249), .(Remarks, Context, SpecialBehaviour, `Got Corn`)])
```
