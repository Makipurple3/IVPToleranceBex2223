...
### Creating a new dataframe - Bex
* Since I do not want to work with the whole dataset, I'm going to select the variables of interest using the function **select**
...
# Check if changes had been madefor Remarks
head(Bex$Remarks)

# Count "No Remarks" and different values
table_remarks <- table(ifelse(Bex$Remarks == "No Remarks", "No Remarks", "Remarks"))
table_remarks

#Creation of a New dichotmoic variable "No Remarks"
Bex$NoRemarks <- ifelse(Bex$Remarks == "No Remarks", 1, 0)

# Add the new column "No Remarks" next to "Remarks"
Bex <- cbind(Bex, NoRemarks = ifelse(Bex$Remarks == "No Remarks", 1, 0))
...
if ("DyadDistance" %in% names(Bex)) {
  ...
  if (!all(c("Dyadistance", "Distance") %in% names(Bex))) {
    ...
    # Check if the "NewDistance" column already exists
    if (!"NewDistance" %in% names(Bex)) {
      ...
      # Create a new column NewDistance with values from DyadDistance
      Bex <- Bex %>% mutate(NewDistance = Dyadistance)

      # Count NA's in NewDistance
      na_count <- sum(is.na(Bex$NewDistance))
      print(paste("Number of NA's in NewDistance: ", na_count))

      # Remove rows with NA in NewDistance after replacing values
      Bex <- Bex %>% filter(!is.na(NewDistance))

      # Count NA's in NewDistance after removing NA's
      na_count <- sum(is.na(Bex$NewDistance))
      print(paste("Number of NA's in NewDistance after removing NA's: ", na_count))
...
Bex$NoTrial <- ifelse(seq_along(Bex$NewDistance) %in% rows_to_delete, 1, 0)
...
* Unique Combinations and Counts:
 - Distracted &  Intrusion  2                   - Intrusion > Distracted: Remove distracted if there is instrusion
 - Female aggress male &  Intrusion 2           - Female aggress male > Intrusion: Remove intrusion if there is Female aggress male
 - Looks at partner &  Other 1                  - **Display line for detailed check**
 - Losing interest &  Intrusion 1               - Intrusion > Losing interest
 - Losing interest &  Looks at partner 1        - Losing interest > Looks at partner ! I have to remove the column **looks at partner** since it was not present in my whole dataset
 - Male aggress female &  Female aggress male 2 - Male aggress female = Female aggress male > keep 2 occurences
 - Male aggress female &  Looks at partner 1    - Male agress female > Looks at partner ! I have to remove the column **looks at partner** since it was not present in my whole dataset
 - Not approaching &  Intrusion 1               - Intrusion > Not approaching: Remove not approaching if there is intrusion
 - Not approaching &  Losing interest 1         - **Keep both**
 - Female aggress male &  Intrusion 1           - Female aggress male > Intrusion: Remove Intrusion if there is Female aggress male
 - Female aggress male &  Not approaching 3     - Female aggress male > Not approaching : Remove Not approaching if there is Female aggress male
 - Male aggress female &  Intrusion 1           - Male agress female > Intrusion: Remove Intrusion if there is Male agress female
 - Male aggress female &  Looks at partner 1    - Male agress female > Looks at partner: ! I have to remove the column **looks at partner** since it was not present in my whole dataset
 - Male aggress female &  Not approaching 2     - Male agress female > Not approaching : Remove Not approaching if there is Male aggress female
 - Not approaching &  Distracted 7              - **Display line for detailed check**
 - Not approaching &  Intrusion 21              - **Keep both**
 - Not approaching &  Looks at partner 9        - Not approaching > Looks at partner: ! I have to remove the column **looks at partner** since it was not present in my whole dataset
 ...
 - Tolerance &  Distracted   3                  - Tolerance > Distracted: Remove Distracted if there is tolerance
 - Tolerance &  Female aggress male 23          - Female agress male > Tolerance: Remove Tolerance if there is Female agress male (I will use these tolerance information since it means the individuals got                                                    corn in most cases the check in more detail in the data since sometimes they did not get both the corn)
 - Tolerance &  Intrusion    41                 - **Keep both**
 - Tolerance &  Looks at partner 25             - Tolerance > Looks at partner :! I have to remove the column **looks at partner** since it was not present in my whole dataset
 - Tolerance &  Losing interest 13              - Tolerance > Loosing interest: Remove Losing interest if there is Tolerance
 - Tolerance &  Male aggress female 31          - Male agress female > Tolerance : Remove Tolerance if there is Male agress male (I will use these tolerance information since it means the individuals got                                                    corn in most cases the check in more detail in the data since sometimes they did not get both the corn)
 - Tolerance &  Not approaching 6               - Tolerance > Not approaching: Remove Not approaching if there is tolerance
 - Tolerance &  Other        8                  - **Display line for detailed check**

* Create a dichotomy variable called **Got Corn** that displays 1 if there is Tolerance and 0 if not. I decided to creat this variable before removing tolerance in double cases (Ex: Tolerance & Female agrees male) since in most cases where there was toelrance the individuals Got Corn. I will alter check the remarks since we mentionned the cases where individuals did not get corn

```{r GotCorn, echo=FALSE}
# Assuming 'DyadResponse' is the name of your variable
Bex$GotCorn <- as.integer(grepl("Tolerance", Bex$DyadResponse))

```
* I also want to remove **Looks at partner** from DyadResponse and Bex

```{r Remove Looks at partner, include=FALSE}
# Remove rows where DyadResponse contains "Looks at partner"
Bex <- Bex[!grepl("Looks at partner", Bex$DyadResponse), ]

# Remove "Looks at partner" from DyadResponse
Bex$DyadResponse <- gsub("Looks at partner", "", Bex$DyadResponse)
...
##### Detailed conditional treatment of DyadResponse

* Before making the changes here is a save of Bex
```{r SaveBex2, include=FALSE}
# Save the original dataframe
saveRDS(Bex, "Bex_original.rds")
```
...
In DyadResponse
  ...
  1.Intrusion > Distracted: Remove distracted if there is intrusion in the same cell(x2)
```{r intrusion > Distracted, echo=FALSE}
# Count occurrences before modifications
before_summary <- nrow(Bex)

# 1. Intrusion > Distracted: Remove distracted if there is intrusion and distracted (x2)
Bex <- Bex[!(grepl("Distracted", Bex$DyadResponse) & grepl("Intrusion", Bex$DyadResponse)), ]

# Count occurrences after modification
after_summary1 <- nrow(Bex)

# Display the change in occurrences
cat("Change in Occurrences for the Chunk (Intrusion > Distracted): ", after_summary1 - before_summary, "\n")

```

  2.Female aggress male > Intrusion: Remove intrusion if there is Female aggress male in the same cell(x2)
```{r Female agress male > Intrusion, echo=FALSE}
# Count occurrences before modifications
before_summary <- nrow(Bex)

# 2. Remove rows with 'Intrusion' if 'Female aggress male' and Intrusion
Bex <- Bex[!(grepl("Intrusion", Bex$DyadResponse) & grepl("Female aggress male", Bex$DyadResponse)), ]

# Count occurrences after modification
after_summary2 <- nrow(Bex)

# Display the change in occurrences
cat("Change in Occurrences for the Chunk (Female aggress male > Intrusion): ", after_summary2 - before_summary, "\n")
```

  
  3.Intrusion > Losing interest: Remove Losing interest if there is Intrusion in the same cell(1x)
```{r Intrusion > Loosing interest, echo=FALSE}
# Count occurrences before modifications
before_summary <- nrow(Bex)

# 3. Remove rows with 'Losing interest' if 'Intrusion' and Loosing interest
Bex <- Bex[!(grepl("Losing interest", Bex$DyadResponse) & grepl("Intrusion", Bex$DyadResponse)), ]

# Count occurrences after modification
after_summary3 <- nrow(Bex)

# Display the change in occurrences
cat("Change in Occurrences for the Chunk (Intrusion > Losing interest): ", after_summary3 - before_summary, "\n")


```


  4.Female aggress male > Not approaching:Remove Not approaching if there is Female aggress male in the same cell (3x)
```{r Female aggress male > Not approaching, echo=FALSE} 
# Count occurrences before modifications
before_summary <- nrow(Bex)

# 4.. Remove rows with 'Not approaching' if 'Female aggress male' is present
Bex <- Bex[!(grepl("Not approaching", Bex$DyadResponse) & grepl("Female aggress male", Bex$DyadResponse)), ]
...
# 9. Remove rows with 'Losing interest' if 'Tolerance' is present
Bex <- Bex[!(grepl("Losing interest", Bex$DyadResponse) & grepl("Tolerance", Bex$DyadResponse)), ]
