...
keywords <- list(
  ...
  code_3_letters = "\\b[a-zA-Z]{3}\\b",  # Regex for 3 letter words (code)
  code_4_letters = "\\b[a-zA-Z]{4}\\b",  # Regex for 4 letter words (code)
  audience = "audience",
  present = "present",
  sec = "sec",
  before = "before",
...
# Function to find rows with keywords
...
results <- lapply(keywords, function(kw, df) {
  if (is.character(kw)) {
    # For single keywords
    rows <- find_keyword_rows(df, kw)
  } else {
    # For regex patterns
    rows <- find_keyword_rows(df, kw)
  }
...
# Adding category names and counting the number of lines for each category
