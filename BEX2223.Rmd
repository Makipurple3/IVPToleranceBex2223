...
* I will keep the following variables in the next order: Date, Month, Day, Time, Period, Trial, Male, Female, Dyad, DyadDistance, Proximity, DyadResponse, Intrusion, IntruderID SpecialBehaviour, Audience, AmountAudience, Context, Special Behaviour
...
BexClean <- BexClean %>%
  select(Date, Month, Day, Time, Hour, Period, Trial, Male, Female, Dyad, 
...
# 1. Improved Overall Male vs Female Aggression Plot

# Calculate overall aggression counts
overall_aggression <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(DyadResponse) %>%
  summarise(Count = n(), .groups = 'drop')

# Calculate the mean number of aggressions for display
mean_agg <- mean(overall_aggression$Count)

# Create the plot
overall_aggression_plot <- ggplot(data = overall_aggression, aes(x = DyadResponse, y = Count, fill = DyadResponse)) +
  geom_bar(stat = "identity", width = 0.5) +
  geom_text(aes(label = Count), vjust = -0.5, size = 5) +  # Add count on top of bars
  theme_minimal(base_size = 15) +
  labs(x = "Aggression Type", y = "Count", title = "Overall Male vs Female Aggression") +
  scale_fill_manual(values = c("Male aggress female" = "#1f77b4", "Female aggress male" = "#ff7f0e")) + # Changed colors
  theme(legend.position = "none") +
  geom_hline(aes(yintercept = mean_agg), linetype = "dashed", color = "gray") +  # Add mean line
  annotate("text", x = 1.5, y = mean_agg + 3, label = paste("Mean =", round(mean_agg, 2)), color = "gray", size = 5)

# Print the overall aggression plot
print(overall_aggression_plot)

# 2. Improved Proportion of Aggression by Dyad Plot

# Improved Proportion of Aggression by Dyad Plot with Updated Colors

# Calculate proportions
dyad_aggression <- BexClean %>%
  filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
  group_by(Dyad, DyadResponse) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  group_by(Dyad) %>%
  mutate(Proportion = Count / sum(Count))  # Calculate proportion

# Horizontal bar plot with percentages
dyad_aggression_plot <- ggplot(data = dyad_aggression, aes(x = Proportion, y = reorder(Dyad, Proportion), fill = DyadResponse)) +
  geom_bar(stat = "identity", position = "fill", width = 0.7) +
  theme_minimal(base_size = 15) +
  labs(x = "Proportion (%)", y = "Dyad", title = "Proportion of Male vs Female Aggression by Dyad") +
  scale_fill_manual(values = c("Male aggress female" = "#1f77b4", "Female aggress male" = "#ff7f0e")) +
  geom_text(aes(label = paste0(round(Proportion * 100, 1), "%")), position = position_fill(vjust = 0.5), size = 4) +
  theme(axis.text.y = element_text(angle = 0, hjust = 1))

# Print the updated dyad aggression proportion plot
print(dyad_aggression_plot)
...
```{r agression & audience test grap, echo=FALSE}


# Update the grouping with correct category order and relabel
BexClean <- BexClean %>%
    mutate(GroupedAudience = case_when(
        as.numeric(AmountAudience) == 0 ~ "0",
        as.numeric(AmountAudience) > 0 & as.numeric(AmountAudience) <= 3 ~ "1-3",
        as.numeric(AmountAudience) > 3 ~ ">3"
    ))

# Ensure the categories are in the correct order
BexClean$GroupedAudience <- factor(BexClean$GroupedAudience, levels = c("0", "1-3", ">3"))

# Calculate counts of aggression types by these audience categories
audience_aggression <- BexClean %>%
    filter(DyadResponse %in% c("Male aggress female", "Female aggress male")) %>%
    group_by(GroupedAudience, DyadResponse) %>%
    summarise(Count = n(), .groups = 'drop')

# Plot the relationship between audience amount and aggression with new colors and label
audience_aggression_plot <- ggplot(data = audience_aggression, aes(x = GroupedAudience, y = Count, fill = DyadResponse)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.7) +
    theme_minimal(base_size = 15) +
    labs(x = "Amount of Audience", y = "Count of Aggressions",
         title = "Relationship Between Audience Amount and Aggression",
         fill = "Aggression Type") +
    scale_fill_manual(values = c("#66c2a5", "#fc8d62")) +  # Teal and Orange colors
    geom_text(aes(label = Count), position = position_dodge(width = 0.7), vjust = -0.3, size = 4)

# Print the updated audience-aggression plot
print(audience_aggression_plot)
